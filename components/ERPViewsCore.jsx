...
export default function ERPViewsCore(props) {
  ...
  // ================= CREATE INVOICE (ADVANCED AUTOMATED FORM) =================
  if (page === 'create') {
      ...
      <div style={styles.card}>
        ...
        <form onSubmit={handleCreateInvoice}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px' }}>
            ...
            {/* Booking Type & Reissue Automation */}
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={styles.label}>{tr.bookingType}</label>
              <select value={invForm.bookingType} onChange={e => setInvForm({...invForm, bookingType: e.target.value, linkedInvId: '', oldTicketNo: '', oldPnr: '', oldAirline: '', oldSector: '', oldSellPrice: 0, oldBookingDate: '', useCredit: 0})} style={styles.input}>
                <option>{tr.newBooking}</option>
                <option>{tr.reissue}</option>
                <option>{tr.extraLuggage}</option>
                <option>{tr.previousBooking}</option>
              </select>
            </div>
            ...
            {invForm.bookingType === 'Previous Booking' && (
              <div style={{ gridColumn: '1 / -1', background: '#F8FAFC', padding: '20px', borderRadius: '12px', border: '1px solid #E2E8F0' }}>
                <h4 style={{ margin: '0 0 15px', color: '#0F172A' }}>Select Previous Refund Invoice (Auto-Fill)</h4>
                ...
                {invForm.linkedInvId && (
                  <div style={{ marginTop: '15px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                    <div style={{ background: '#fff', padding: '10px', borderRadius: '8px', border: '1px solid #fde68a' }}>
                      <div style={{ fontSize: '11px', color: '#92400e', fontWeight: 600 }}>Original Airline</div>
                      <div style={{ fontSize: '14px', color: '#78350f', fontWeight: 700 }}>{invForm.oldAirline || 'N/A'}</div>
                    </div>
                    <div style={{ background: '#fff', padding: '10px', borderRadius: '8px', border: '1px solid #fde68a' }}>
                      <div style={{ fontSize: '11px', color: '#92400e', fontWeight: 600 }}>Original Sector</div>
                      <div style={{ fontSize: '14px', color: '#78350f', fontWeight: 700 }}>{invForm.oldSector || 'N/A'}</div>
                    </div>
                    <div style={{ background: '#fff', padding: '10px', borderRadius: '8px', border: '1px solid #fde68a' }}>
                      <div style={{ fontSize: '11px', color: '#92400e', fontWeight: 600 }}>Original Ticket No</div>
                      <div style={{ fontSize: '14px', color: '#78350f', fontWeight: 700 }}>{invForm.oldTicketNo || 'N/A'}</div>
                    </div>
                    <div style={{ background: '#fff', padding: '10px', borderRadius: '8px', border: '1px solid #fde68a' }}>
                      <div style={{ fontSize: '11px', color: '#92400e', fontWeight: 600 }}>Original Fare</div>
                      <div style={{ fontSize: '14px', color: '#78350f', fontWeight: 700 }}>{parseFloat(invForm.oldSellPrice || 0).toFixed(2)} SAR</div>
                    </div>
                    <div style={{ background: '#dcfce7', padding: '10px', borderRadius: '8px', border: '1px solid #86efac' }}>
                      <div style={{ fontSize: '11px', color: '#059669', fontWeight: 600 }}>Refund Credit Applied</div>
                      <div style={{ fontSize: '14px', color: '#047857', fontWeight: 700 }}>- {creditUsed.toFixed(2)} SAR</div>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div style={{ gridColumn: '1 / -1', background: '#0F172A', color: 'white', padding: '20px', borderRadius: '12px', marginTop: '10px' }}>
              <h4 style={{ margin: '0 0 15px', color: '#F59E0B' }}>Live Payment Calculation</h4>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px', margin: '5px 0' }}><span>New Booking Price:</span><span>{totalNewBooking.toFixed(2)} SAR</span></div>
              {creditUsed > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px', margin: '5px 0', color: '#34d399' }}><span>Less: Refund Credit Applied:</span><span>- {creditUsed.toFixed(2)} SAR</span></div>}
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '16px', margin: '10px 0 0', fontWeight: 'bold', borderTop: '1px solid #334155', paddingTop: '10px' }}>
            ...
            <div><label style={styles.label}>{tr.paidAmount} (Cash/Bank)</label><input type="number" step="0.01" value={invForm.paid} onChange={e => setInvForm({...invForm, paid: e.target.value})} style={styles.input} required /></div>
