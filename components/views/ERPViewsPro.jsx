...
export default function ERPViewsPro(props) {
  ...
  if (page === 'refund_statement') return <RefundStatementView {...props} />;
  if (page === 'customer_statement') return <CustomerStatementView {...props} />;
  if (page === 'supplier_statement') return <SupplierStatementView {...props} />;
  if (page === 'recurring_invoices') return <RecurringInvoicesView {...props} />;
  if (page === 'expense_approval') return <ExpenseApprovalView {...props} />;
  if (page === 'notifications') return <NotificationsView {...props} />;
...
// ============================================================
// 1. REFUND STATEMENT VIEW
// ============================================================
function RefundStatementView(props) {
...
function CustomerStatementView(props) {
  const { data, tr, today, userProfile, showToast, handleExportCSV, lang, theme } = props;
  const { styles, fmt } = useProHelpers(props);
  const isAr = lang === 'ar';
  const isDark = theme === 'dark';
...
// ============================================================
// 3. SUPPLIER STATEMENT VIEW
// ============================================================
function SupplierStatementView(props) {
  const { data, tr, today, userProfile, showToast, handleExportCSV, lang, theme } = props;
  const { styles, fmt } = useProHelpers(props);
  const isAr = lang === 'ar';
  const isDark = theme === 'dark';
...
function RecurringInvoicesView(props) {
  ...
  useEffect(() => {
    if (userProfile?.tenant_id) loadRecurring();
        ...
        <form onSubmit={handleCreateRecurring} style={styles.formRow}>
          ...
          <div>
            ...
            <select style={styles.select} value={recForm.interval} onChange={e => setRecForm({ ...recForm, interval: e.target.value })}>
              <option>Monthly</option><option>Yearly</option><option>Weekly</option><option>Quarterly</option>
            </select>
          </div>
          <div>
...
// ============================================================
// 5. EXPENSE APPROVAL VIEW
// ============================================================
function ExpenseApprovalView(props) {
  const { data, setData, showToast, lang, theme, userProfile } = props;
  const { styles, fmt } = useProHelpers(props);
  const isAr = lang === 'ar';
  const isDark = theme === 'dark';
...
// ============================================================
// 7. STAFF MISTAKES VIEW (Fixed controlled form)
// ============================================================
function StaffMistakesView(props) {
  const {
    data, lang, theme, showToast, setData, userProfile,
    handleAddMistake, handlePreviewMistake, handleDeleteMistake,
    mistakeForm, setMistakeForm, today, getMistakeHTML, downloadPDF, setPreviewHTML, setModal
  } = props;
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>⚠️ {isAr ? 'أخطاء الموظفين وتتبع الخسائر' : 'Staff Mistakes & Loss Tracking'}</h1>
