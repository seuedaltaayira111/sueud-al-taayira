...
export default function ERPViewsEnterprise(props) {
  const {
    ...
    handleExportCSV, setPage
  } = props;

  // Route to appropriate component
  if (page === 'credit_limits') return <CreditLimitsView {...props} />;
  if (page === 'supplier_statement') return <SupplierStatementView {...props} />;
  if (page === 'multi_branch') return <MultiBranchView {...props} />;
  return null;
}
...
// SHARED HELPERS
// ============================================================
function useEnterpriseHelpers(props) {
...
// ============================================================
// 1. CREDIT LIMITS VIEW
// ============================================================
function CreditLimitsView(props) {
  const { data, setData, showToast, lang, theme } = props;
  const { styles, fmt } = useEnterpriseHelpers(props);
  const isAr = lang === 'ar';
  const isDark = theme === 'dark';
...
// ============================================================
// 2. SUPPLIER STATEMENT VIEW (Enterprise version)
// ============================================================
...
// 3. MULTI-BRANCH VIEW
// ============================================================
function MultiBranchView(props) {
  const { data, tr, lang, theme, setPage } = props;
  const { styles, fmt } = useEnterpriseHelpers(props);
  const isAr = lang === 'ar';
  const isDark = theme === 'dark';
      ...
      {(data.branches || []).length === 0 ? (
        <div style={styles.card}>
          <div style={styles.emptyState}>
            ...
            <button style={{ ...styles.btn, ...styles.btnPrimary, marginTop: '15px' }} onClick={() => setPage('branches')}>
                  ...
                  <div style={{ marginTop: '15px', paddingTop: '15px', borderTop: isDark ? '1px solid #334155' : '1px solid #E2E8F0' }}>
                    ...
                    <div style={{ marginTop: '10px', display: 'flex', gap: '10px' }}>
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); setPage('branches'); }}
                      >
