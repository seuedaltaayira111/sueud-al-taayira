      ...
      <div style={styles.container}>
        ...
        <div style={styles.card}>
          ...
          <form onSubmit={handleAddEditUser} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '15px' }}>
            ...
            <div><label style={styles.formLabel}>Link to Employee</label><select style={styles.select} value={userForm?.employee_id || ''} onChange={e => setUserForm(prev => ({ ...prev, employee_id: e.target.value }))}><option value="">— None —</option>{(data.employees || []).map(emp => <option key={emp.id} value={emp.id}>{emp.name}</option>)}</select></div>
    ...
    return (
      <div style={styles.container}>
        ...
        <div style={styles.card}>
          ...
          <tbody>
            {(data.customers || []).filter(c => !searchTerm || c.name?.toLowerCase().includes(searchTerm.toLowerCase())).map(c => {
              const available = (c.credit_limit || 0) - ((c.store_credit || 0) > 0 ? 0 : c.store_credit || 0);
              return (
                <tr key={c.id}>
                  <td style={{ ...styles.td, fontWeight: '600' }}>{c.name}</td>
                  <td style={styles.td}>{c.phone || '-'}</td>
                  <td style={{ ...styles.tdRight, color: '#7C3AED' }}>{fmt(c.store_credit)}</td>
                  <td style={{ ...styles.tdRight, color: '#2563EB' }}>{fmt(c.credit_limit)}</td>
                  <td style={{ ...styles.tdRight, color: available >= 0 ? '#059669' : '#EF4444' }}>{fmt(available)}</td>
                </tr>
              );
            })}
          </tbody></table>
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        <div style={styles.emptyState}>
          ...
          <h2 style={{ color: '#374151', marginBottom: '10px' }}>Page: {page}</h2>
          <p style={{ color: '#6B7280' }}>This section is under development.</p>
          <button style={{ ...styles.btn, ...styles.btnPrimary, marginTop: '20px' }} onClick={() => setPage('list')}>← Back to Invoices</button>
        </div>
      </div>
    </div>
  );
