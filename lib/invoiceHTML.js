...
/* ================================================================
   INVOICE HTML GENERATORS - BILINGUAL (EN/AR) - BEAUTIFUL DESIGN
...
// ================================================================
// 1. SALES INVOICE - Bilingual, Beautiful
// ================================================================
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; padding: 0; }
    ...
    body {
      font-family: ${lang === 'ar' ? "'Cairo', 'Tajawal', sans-serif" : "'Inter', 'Poppins', sans-serif"};
      background: #F8FAFC;
      color: #1E293B;
      font-size: 10px;
      display: flex;
...
<div class="invoice-container">
  <div class="header">
    ...
    <div class="header-right">
      <div class="doc-title">${t('TAX INVOICE', 'فاتورة ضريبية')}</div>
      <div class="meta">${t('No', 'رقم')}: <span>${no}</span></div>
      <div class="meta">${t('Date', 'التاريخ')}: <span>${inv.invoice_date || ''}</span></div>
      <div class="status-badge">${st2 === 'Unpaid' ? t('UNPAID', 'غير مدفوعة') : t('PAID', 'مدفوعة')}</div>
    </div>
  </div>
  <div class="company-bar">
    <div class="item"><div class="label">${t('VAT No', 'رقم الضريبة')}</div><div class="value">${st.vat_no || 'N/A'}</div></div>
    <div class="item"><div class="label">${t('CR No', 'رقم السجل')}</div><div class="value">${st.cr_no || 'N/A'}</div></div>
    <div class="item"><div class="label">${t('License', 'الترخيص')}</div><div class="value">${st.license_no || 'N/A'}</div></div>
    <div class="item"><div class="label">${t('Tourism Lic.', 'رخصة سياحة')}</div><div class="value">${st.tourism_license_no || 'N/A'}</div></div>
  </div>
  <div class="body">
    ${storeCredit > 0 ? `<div class="store-credit"><span>💰 ${t('Available Store Credit', 'الرصيد المتاح')}</span><span class="value">${storeCredit.toFixed(2)} SAR</span></div>` : ''}
    <div class="info-grid">
      <div class="info-box">
        <div class="section-title">${t('BILL TO', 'فاتورة إلى')}</div>
        <div class="info-row"><span class="label">${t('Name', 'الاسم')}</span><span class="value">${inv.customers?.name || inv.corporates?.name || 'N/A'}</span></div>
        <div class="info-row"><span class="label">${t('Phone', 'الهاتف')}</span><span class="value">${inv.customers?.phone || 'N/A'}</span></div>
        ${inv.corporates?.vat_no ? `<div class="info-row"><span class="label">${t('VAT', 'الضريبة')}</span><span class="value">${inv.corporates.vat_no}</span></div>` : ''}
        <div class="info-row"><span class="label">${t('Sales Person', 'موظف المبيعات')}</span><span class="value">${inv.employees?.name || 'N/A'}</span></div>
        <div class="info-row"><span class="label">${t('Passengers', 'الركاب')}</span><span class="value" style="font-size:8px;">${pax}</span></div>
      </div>
      <div class="info-box gold">
        <div class="section-title">${t('FLIGHT DETAILS', 'تفاصيل الرحلة')}</div>
        <div class="info-row"><span class="label">${t('Airline', 'الخطوط')}</span><span class="value">${inv.airline || 'N/A'}</span></div>
        <div class="info-row"><span class="label">${t('Sector', 'القطاع')}</span><span class="value">${inv.flight_sector || 'N/A'}</span></div>
        <div class="info-row"><span class="label">${t('Type', 'النوع')}</span><span class="value">${inv.flight_type || 'N/A'}</span></div>
        <div class="info-row"><span class="label">${t('Journey', 'الرحلة')}</span><span class="value">${inv.flight_journey || 'N/A'}</span></div>
        <div class="info-row"><span class="label">${t('PNR', 'رقم الحجز')}</span><span class="value" style="color:#2563EB;font-weight:700;">${inv.pnr || 'N/A'}</span></div>
        <div class="info-row"><span class="label">${t('Ticket No', 'رقم التذكرة')}</span><span class="value">${inv.ticket_no || 'N/A'}</span></div>
        <div class="info-row"><span class="label">${t('Refundable', 'قابل للاسترجاع')}</span><span class="value">${inv.refundable_status || 'N/A'}</span></div>
        <div class="info-row"><span class="label">${t('Service', 'الخدمة')}</span><span class="value">${inv.service_type || 'N/A'}</span></div>
      </div>
    </div>
    <table class="invoice-table">
      ...
      <tbody>
        ...
        ${disc > 0 ? `<tr class="discount-row"><td colspan="3" style="text-align:right;color:#059669;">${t('Discount', 'خصم')}</td><td class="right" style="color:#059669;">- ${disc.toFixed(2)}</td></tr>` : ''}
      </tbody>
    </table>
    <div class="bottom-section">
      <div class="payment-breakdown">
        <div class="section-title">${t('PAYMENT BREAKDOWN', 'تفصيل الدفع')}</div>
        <div class="pay-row"><span class="label">${t('Subtotal', 'المجموع الفرعي')}</span><span class="value">${sub.toFixed(2)} SAR</span></div>
        ${disc > 0 ? `<div class="pay-row" style="color:#059669;"><span class="label">${t('Discount', 'خصم')}</span><span class="value">- ${disc.toFixed(2)} SAR</span></div>` : ''}
        <div class="pay-row"><span class="label">${t('VAT', 'الضريبة')} (${vr}%)</span><span class="value">${vat.toFixed(2)} SAR</span></div>
        ${uc > 0 ? `<div class="pay-row" style="color:#7C3AED;"><span class="label">${t('Credit Used', 'الرصيد المستخدم')}</span><span class="value">- ${uc.toFixed(2)} SAR</span></div>` : ''}
        ${cr > 0 ? `<div class="pay-row" style="color:#EF4444;"><span class="label">${t('Cash Returned', 'النقد المرتجع')}</span><span class="value">- ${cr.toFixed(2)} SAR</span></div>` : ''}
        <div class="pay-row" style="border-top:2px solid #CBD5E1;margin-top:4px;padding-top:4px;font-weight:700;">
          <span class="label">${t('Paid', 'المدفوع')} (${pd})</span>
          <span class="value green">${cp.toFixed(2)} SAR</span>
        </div>
        <div class="pay-row" style="font-weight:700;font-size:11px;">
          <span class="label">${t('Due', 'المتبقي')}</span>
          <span class="value ${due > 0 ? 'red' : 'green'}">${due.toFixed(2)} SAR</span>
        </div>
      </div>
      <div class="totals-box">
        <div class="total-row"><span>${t('Subtotal', 'المجموع الفرعي')}</span><span>${sub.toFixed(2)}</span></div>
        ${disc > 0 ? `<div class="total-row" style="color:#34D399;"><span>${t('Discount', 'خصم')}</span><span>- ${disc.toFixed(2)}</span></div>` : ''}
        <div class="total-row"><span>${t('VAT', 'الضريبة')} (${vr}%)</span><span>${vat.toFixed(2)}</span></div>
        <div class="grand-total"><span>${t('GRAND TOTAL', 'الإجمالي الكلي')}</span><span class="value">${tot.toFixed(2)} SAR</span></div>
      </div>
    </div>
  </div>
  <div class="footer">
    <div class="code-section">
      ...
      <div class="code-label">${t('CHECK-IN', 'تسجيل الوصول')}</div>
      <img src="${qr}" alt="QR" class="qr"/>
      <div class="code-label">${t('INVOICE', 'الفاتورة')}</div>
    </div>
    <div class="ai-message">
      <div class="ai-label">🤖 AI</div>
      <p>${lang === 'ar' ? aiMsg.ar : aiMsg.en}</p>
    </div>
  ...
  <div class="linked-section">
    <div class="linked-title">🔄 ${t('Previous Booking & Linked Refund', 'الحجز السابق والاسترجاع المرتبط')}</div>
    <div class="reissue-block">
      <div class="reissue-item"><div class="label">${t('Old Date', 'التاريخ القديم')}</div><div class="value">${prevDate}</div></div>
      <div class="reissue-item"><div class="label">${t('Old Airline', 'الخطوط السابقة')}</div><div class="value">${prevAirline}</div></div>
      <div class="reissue-item"><div class="label">${t('Old Sector', 'القطاع السابق')}</div><div class="value">${prevSector}</div></div>
      <div class="reissue-item"><div class="label">${t('Old PNR', 'رقم الحجز القديم')}</div><div class="value">${prevPnr}</div></div>
      <div class="reissue-item"><div class="label">${t('Old Ticket', 'التذكرة القديمة')}</div><div class="value">${prevTicket}</div></div>
      <div class="reissue-item"><div class="label">${t('Old Type', 'النوع القديم')}</div><div class="value">${prevType}</div></div>
      <div class="reissue-fare"><span class="label">${t('Original Fare', 'السعر الأصلي')}</span><span class="value">${prevFare.toFixed(2)} SAR</span></div>
    </div>
    <div class="refund-details">
      <div class="refund-item"><div class="label">${t('Refund Amount', 'مبلغ الاسترجاع')}</div><div class="value">${prevRefundCust.toFixed(2)} SAR</div></div>
      <div class="refund-item"><div class="label">${t('Refund Date', 'تاريخ الاسترجاع')}</div><div class="value">${inv.refund_date || inv.invoice_date || 'N/A'}</div></div>
      <div class="refund-item"><div class="label">${t('Reason', 'السبب')}</div><div class="value">${prevRefundReason || 'N/A'}</div></div>
    </div>
  </div>` : ''}
</div>
</body>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; padding: 0; }
    ...
    .header-right .meta { font-size: 9px; margin-top: 4px; color: rgba(255,255,255,0.8); }
    .header-right .meta span { color: #FBBF24; font-weight: 700; }
    .status-badge { display: inline-block; padding: 3px 12px; border-radius: 20px; font-weight: 700; font-size: 9px; margin-top: 6px; background: rgba(251,191,36,0.2); color: #FBBF24; border: 1px solid rgba(251,191,36,0.3); }
    ...
    .section-title { font-size: 10px; font-weight: 700; text-transform: uppercase; color: #7F1D1D; border-bottom: 2px solid #7F1D1D; padding-bottom: 4px; margin-bottom: 6px; display: flex; justify-content: space-between; }
    .info-box { padding: 8px 12px; background: #FEF2F2; border-radius: 8px; border-left: 4px solid #DC2626; }
    .info-row { display: flex; justify-content: space-between; font-size: 9px; padding: 2px 0; border-bottom: 1px solid #FECACA; }
    .info-row:last-child { border-bottom: none; }
    ...
    .refund-summary { background: #FFF; padding: 10px 14px; border-radius: 8px; border: 1px solid #E2E8F0; }
    .refund-row { display: flex; justify-content: space-between; font-size: 9px; padding: 3px 0; border-bottom: 1px dashed #E2E8F0; }
    .refund-row:last-child { border-bottom: none; }
    .refund-row .label { color: #64748B; }
    .refund-row .value { font-weight: 600; }
    .refund-row.total { border-top: 2px solid #DC2626; font-size: 14px; font-weight: 800; color: #059669; }
    .refund-row.total .label { color: #334155; }
    .refund-row.total .value { color: #059669; }
    .refund-method { padding: 8px 14px; background: #F8FAFC; border-radius: 8px; border: 1px solid #E2E8F0; display: flex; justify-content: space-between; align-items: center; font-size: 9px; }
    .refund-method .label { color: #334155; font-weight: 600; }
    .refund-method .value { color: #2563EB; font-weight: 700; }
    .footer { padding: 8px 24px; background: #FEF2F2; border-top: 1px solid #FECACA; display: flex; justify-content: space-between; align-items: center; flex-shrink: 0; margin-top: auto; gap: 12px; }
    .code-section { display: flex; align-items: center; gap: 6px; }
    ...
    .ai-message { flex: 1; text-align: center; padding: 4px 12px; background: #FEF2F2; border-radius: 6px; border: 1px solid #FCA5A5; }
    ...
    @media print { body { background: white; padding: 0; margin: 0; } .refund-container { border-radius: 0; box-shadow: none; height: 100%; width: 100%; } }
  </style>
</head>
<body>
<div class="refund-container">
  <div class="header">
    ...
    <div class="header-right">
      <div class="doc-title">${t('REFUND', 'استرجاع')}</div>
      <div class="meta">${t('No', 'رقم')}: <span>${no}</span></div>
      <div class="meta">${t('Date', 'التاريخ')}: <span>${inv.refund_date || inv.invoice_date || ''}</span></div>
      <div class="status-badge">${t('PROCESSED', 'تمت المعالجة')}</div>
    </div>
  </div>
  <div class="company-bar">
    <div class="item"><div class="label">${t('VAT No', 'رقم الضريبة')}</div><div class="value">${st.vat_no || 'N/A'}</div></div>
  ...
  <div class="body">
    <div class="info-box">
      <div class="section-title">${t('BOOKING DETAILS', 'تفاصيل الحجز')}</div>
      <div class="info-row"><span class="label">${t('Customer', 'العميل')}</span><span class="value">${cn}</span></div>
      <div class="info-row"><span class="label">${t('Phone', 'الهاتف')}</span><span class="value">${cp}</span></div>
      <div class="info-row"><span class="label">${t('Passengers', 'الركاب')}</span><span class="value" style="font-size:8px;">${pax}</span></div>
      <div class="info-row"><span class="label">${t('Airline', 'الخطوط')}</span><span class="value">${inv.airline || inv.old_airline || 'N/A'}</span></div>
      <div class="info-row"><span class="label">${t('Date', 'التاريخ')}</span><span class="value">${inv.invoice_date || 'N/A'}</span></div>
      <div class="info-row"><span class="label">${t('PNR', 'رقم الحجز')}</span><span class="value" style="color:#2563EB;font-weight:700;">${inv.pnr || inv.old_pnr || 'N/A'}</span></div>
      <div class="info-row"><span class="label">${t('Reason', 'السبب')}</span><span class="value">${inv.refund_reason || 'N/A'}</span></div>
    </div>
    <div class="refund-summary">
      <div class="section-title">${t('REFUND SUMMARY', 'ملخص الاسترجاع')}</div>
      <div class="refund-row"><span class="label">${t('Original Fare', 'السعر الأصلي')}</span><span class="value">${of2.toFixed(2)} SAR</span></div>
      <div class="refund-row total"><span class="label">${t('Refund Amount', 'مبلغ الاسترجاع')}</span><span class="value">${cRef.toFixed(2)} SAR</span></div>
    </div>
    <div class="refund-method">
      <span class="label">${t('Refund Method', 'طريقة الاسترجاع')}</span>
  ...
  <div class="footer">
    <div class="code-section">
      <img src="${barcode}" alt="Barcode"/>
      <div class="code-label">${t('AIRLINE INFO', 'معلومات الخطوط')}</div>
      <img src="${qr}" alt="QR" class="qr"/>
      <div class="code-label">${t('DOWNLOAD', 'تحميل')}</div>
    </div>
    <div class="ai-message">
      <div class="ai-label">🤖 AI</div>
      <p>${lang === 'ar' ? aiMsg.ar : aiMsg.en}</p>
...
<div class="expense-container">
  <div class="header">
    ...
    <div class="header-right">
      <div class="doc-title">${t('EXPENSE VOUCHER', 'سند صرف')}</div>
      <div class="meta">${t('No', 'رقم')}: <span>${eno}</span></div>
      <div class="meta">${t('Date', 'التاريخ')}: <span>${exp.expense_date || ''}</span></div>
    </div>
  </div>
  <div class="company-bar">
    <div class="item"><div class="label">${t('VAT No', 'رقم الضريبة')}</div><div class="value">${st.vat_no || 'N/A'}</div></div>
  ...
  <div class="body">
    <div class="info-box">
      <div class="section-title">${t('DETAILS', 'التفاصيل')}</div>
      <div class="info-row"><span class="label">${t('Type', 'النوع')}</span><span class="value">${exp.expense_type || 'N/A'}</span></div>
      <div class="info-row"><span class="label">${t('Payment', 'طريقة الدفع')}</span><span class="value">${exp.payment_mode || 'Cash'}</span></div>
      <div class="info-row"><span class="label">${t('Description', 'الوصف')}</span><span class="value">${exp.description || 'N/A'}</span></div>
    </div>
    <table class="expense-table">
      <thead><tr><th>${t('Item', 'العنصر')}</th><th class="right">${t('Qty', 'الكمية')}</th><th class="right">${t('Price', 'السعر')}</th><th class="right">${t('Total', 'الإجمالي')}</th></tr></thead>
      <tbody>
  ...
  <div class="footer">
    <div class="code-section">
      <img src="${barcode}" alt="Barcode"/>
      <div class="code-label">${t('EXPENSE REF', 'مرجع المصروف')}</div>
      <img src="${qr}" alt="QR" class="qr"/>
      <div class="code-label">${t('VERIFY', 'تحقق')}</div>
    </div>
    <div class="ai-message">
      <div class="ai-label">🤖 AI</div>
      <p>${lang === 'ar' ? aiMsg.ar : aiMsg.en}</p>
...
<div class="slip-container">
  <div class="header">
    ...
    <div class="header-right">
      <div class="doc-title">${t('SALARY SLIP', 'قسيمة راتب')}</div>
      <div class="meta">${t('No', 'رقم')}: <span>${sno}</span></div>
      <div class="meta">${t('Month', 'الشهر')}: <span>${pay.month || 'N/A'}</span></div>
      <div class="meta">${t('Employee', 'الموظف')}: <span>${pay.employees ? pay.employees.name : 'N/A'}</span></div>
    </div>
  </div>
  <div class="company-bar">
    <div class="item"><div class="label">${t('VAT No', 'رقم الضريبة')}</div><div class="value">${st.vat_no || 'N/A'}</div></div>
  ...
  <div class="body">
    <div class="salary-grid">
      <div class="salary-block earnings">
        <div class="title">${t('EARNINGS', 'الإستحقاقات')}</div>
        <div class="salary-row"><span class="label">${t('Base Salary', 'الراتب الأساسي')}</span><span class="value">${(pay.salary || pay.base_salary || 0).toFixed(2)} SAR</span></div>
        <div class="salary-row"><span class="label">${t('Commission', 'العمولة')}</span><span class="value">${(pay.commission_amount || pay.commission || 0).toFixed(2)} SAR</span></div>
        <div class="salary-row"><span class="label">${t('Overtime', 'العمل الإضافي')}</span><span class="value">${(pay.overtime || 0).toFixed(2)} hrs</span></div>
        <div class="salary-row"><span class="label">${t('Gift/Bonus', 'هدية/مكافأة')}</span><span class="value">${(pay.gift || 0).toFixed(2)} SAR</span></div>
      </div>
      <div class="salary-block deductions">
        <div class="title">${t('DEDUCTIONS', 'الخصومات')}</div>
        <div class="salary-row"><span class="label">${t('Advance Deduct', 'خصم السلفة')}</span><span class="value red">${(pay.advance_deduction || 0).toFixed(2)} SAR</span></div>
        <div class="salary-row"><span class="label">${t('Mistakes Deduct', 'خصم الأخطاء')}</span><span class="value red">${(pay.mistakes_deduction || 0).toFixed(2)} SAR</span></div>
        <div class="salary-row"><span class="label">${t('Other Deduct', 'خصومات أخرى')}</span><span class="value red">${(pay.other_deduction || 0).toFixed(2)} SAR</span></div>
      </div>
      <div class="salary-block net">
        <div class="title">${t('NET PAY', 'صافي الراتب')}</div>
  ...
  <div class="footer">
    <div class="code-section">
      <img src="${barcode}" alt="Barcode"/>
      <div class="code-label">${t('SLIP REF', 'مرجع القسيمة')}</div>
      <img src="${qr}" alt="QR" class="qr"/>
      <div class="code-label">${t('VERIFY', 'تحقق')}</div>
    </div>
    <div class="ai-message">
...
export const getContractHTML = (settings, corpName, date, isOffer = false, serviceType = 'Flight Tickets', markup = '10', terms = '') => {
  ...
  const t = (en, ar) => ar; // we'll use both in labels
...
<div class="contract-container">
  <div class="header">
    ...
    <div class="header-right">
      <div class="doc-title">${isOffer ? 'CORPORATE OFFER' : 'CORPORATE CONTRACT'}</div>
...
<div class="mistake-container">
  <div class="header">
    ...
    <div class="header-right">
      <div class="doc-title">${t('STAFF MISTAKE', 'خطأ الموظف')}</div>
      <div class="meta">${t('Date', 'التاريخ')}: <span>${m.date || ''}</span></div>
      <div class="meta">${t('Ref', 'مرجع')}: <span>${m.id ? 'MS-' + m.id.substring(0, 8) : 'N/A'}</span></div>
    </div>
  </div>
  <div class="company-bar">
    <div class="item"><div class="label">${t('VAT No', 'رقم الضريبة')}</div><div class="value">${st.vat_no || 'N/A'}</div></div>
  ...
  <div class="body">
    <div class="title">⚠️ ${t('Staff Mistake', 'خطأ الموظف')}</div>
    <div>
      <div class="section-title">${t('Mistake Details', 'تفاصيل الخطأ')}</div>
      <div class="info-grid">
        <div class="info-item"><div class="label">${t('Employee', 'الموظف')}</div><div class="value">${m.employees?.name || 'N/A'}</div></div>
        <div class="info-item"><div class="label">${t('Old Ticket', 'التذكرة القديمة')}</div><div class="value">${m.old_ticket_no || 'N/A'}</div></div>
        <div class="info-item"><div class="label">${t('New Ticket', 'التذكرة الجديدة')}</div><div class="value">${m.new_ticket_no || 'N/A'}</div></div>
        <div class="info-item"><div class="label">${t('Reason', 'السبب')}</div><div class="value">${m.reason || 'N/A'}</div></div>
      </div>
    </div>
    <div class="danger-box">
      <div class="danger-title">⚠️ ${t('FINANCIAL IMPACT', 'الأثر المالي')}</div>
      <div class="danger-row"><span>${t('Ticket Price', 'سعر التذكرة')}</span><span>${parseFloat(m.old_sell_price || 0).toFixed(2)} SAR</span></div>
      <div class="danger-row"><span>${t('Airline Fees', 'رسوم الخطوط')}</span><span>- ${((m.old_sell_price || 0) - (m.refund_company || 0)).toFixed(2)} SAR</span></div>
      <div class="danger-row"><span>${t('Customer Refund', 'استرجاع العميل')}</span><span>${(m.refund_customer || 0).toFixed(2)} SAR</span></div>
    </div>
    <div class="loss-amount">${t('LOSS', 'الخسارة')}: ${(m.loss_amount || 0).toFixed(2)} SAR</div>
    <div>
      <div class="section-title">${t('Deduction', 'الخصم')}</div>
      <div class="info-grid">
        <div class="info-item"><div class="label">${t('Loss Amount', 'قيمة الخسارة')}</div><div class="value" style="color:#DC2626;">${(m.loss_amount || 0).toFixed(2)} SAR</div></div>
        <div class="info-item"><div class="label">${t('Deduct from Salary', 'خصم من الراتب')}</div><div class="value" style="color:${m.paid_by_employee ? '#059669' : '#64748B'};">${m.paid_by_employee ? t('YES', 'نعم') : t('NO', 'لا')}</div></div>
      </div>
    </div>
    <div class="ai-box">
      <div class="ai-label">🤖 AI ${t('INSIGHT', 'تحليل')}</div>
      <p>${lang === 'ar' ? aiMsg.ar : aiMsg.en}</p>
    </div>
  </div>
  <div class="footer">
    <p>© ${new Date().getFullYear()} ${st.company_name_en || 'SUEUD AL TAAYIRA'}</p>
    <p style="font-size:8px;color:#94A3B8">${t('Computer-generated - Valid without signature', 'منشأ آلي - ساري بدون توقيع')}</p>
  </div>
</div>
</body>
</html>`;
...
<div class="recharge-container">
  <div class="header">
    ...
    <div class="header-right">
      <div class="doc-title">${t('RECHARGE VOUCHER', 'سند إعادة شحن')}</div>
      <div class="meta">${t('No', 'رقم')}: <span>${no}</span></div>
      <div class="meta">${t('Date', 'التاريخ')}: <span>${date}</span></div>
    </div>
  </div>
  <div class="company-bar">
    <div class="item"><div class="label">${t('VAT No', 'رقم الضريبة')}</div><div class="value">${st.vat_no || 'N/A'}</div></div>
  ...
  <div class="body">
    <div class="info-box">
      <div class="section-title">${t('DETAILS', 'التفاصيل')}</div>
      <div class="info-row"><span class="label">${t('Portal', 'البوابة')}</span><span class="value">${portalName}</span></div>
      <div class="info-row"><span class="label">${t('Source', 'المصدر')}</span><span class="value">${source}</span></div>
      <div class="info-row"><span class="label">${t('Reference', 'المرجع')}</span><span class="value">${reference}</span></div>
      <div class="info-row"><span class="label">${t('Notes', 'ملاحظات')}</span><span class="value">${notes}</span></div>
    </div>
    <div class="total-box">
