...
/* ================================================================
   INVOICE HTML GENERATORS - COMPLETE MODERN DESIGN
   All invoices: Sales, Refund, Expense, Salary, Contract, Mistake, Recharge
   Bilingual: English + Arabic
   Full page fill (no blank space at bottom)
   ================================================================ */
...
/* ================================================================
   SALES INVOICE HTML - MODERN DESIGN (FULL PAGE FILL)
   ================================================================ */
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; padding: 0; }
    ...
    .logo-wrapper .logo-placeholder { font-size: 32px; }
    .company-text h2 {
      font-size: 18px;
      font-weight: 900;
      color: #FBBF24;
    ...
    .invoice-meta h3 span { font-size: 10px; font-weight: 600; opacity: 0.7; display: block; margin-top: 2px; }
    .meta-row {
      display: flex;
      justify-content: space-between;
      font-size: 9px;
    ...
    .meta-row .label { color: rgba(255,255,255,0.6); }
    .meta-row .value { color: #FBBF24; font-weight: 700; }
    .status-badge {
    ...
    .detail-item .label { font-size: 7px; color: #64748B; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
    .detail-item .value { font-size: 9px; color: #0F172A; font-weight: 700; }
    .body {
      padding: 16px 24px;
      flex: 1;
      display: flex;
    ...
    .info-block.gold { border-left-color: #F59E0B; }
    .info-row {
      display: flex;
      justify-content: space-between;
      font-size: 8px;
    ...
    .info-row .value { color: #0F172A; font-weight: 600; text-align: ${isRTL ? 'left' : 'right'}; max-width: 65%; word-break: break-word; }
    .info-row .value.highlight { color: #2563EB; }
    .store-credit-box {
    ...
    .store-credit-box .label { color: #065F46; font-weight: 600; font-size: 9px; }
    .store-credit-box .value { color: #047857; font-weight: 800; font-size: 12px; }
    .linked-section {
      margin-top: 8px;
      padding-top: 10px;
    ...
    .linked-section .title { text-align: center; color: #7F1D1D; font-size: 14px; font-weight: 800; margin-bottom: 10px; }
    .reissue-block {
      padding: 10px 14px;
    ...
    .reissue-item .value { font-size: 8px; color: #78350F; font-weight: 700; margin-top: 1px; }
    .reissue-fare {
      background: #D1FAE5;
      border-color: #6EE7B7;
      grid-column: span 3;
    ...
    .refund-details .refund-title { font-size: 9px; font-weight: 700; color: #DC2626; margin-bottom: 6px; display: flex; justify-content: space-between; }
    .refund-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; }
    .refund-grid .item { text-align: center; }
    .refund-grid .item .label { color: #991B1B; font-size: 7px; }
    .refund-grid .item .value { font-weight: 700; color: #DC2626; font-size: 10px; }
    .invoice-table {
      width: 100%;
    ...
    .invoice-table thead th span { font-family: 'Cairo', sans-serif; font-size: 7px; opacity: 0.7; display: block; font-weight: 400; }
    .invoice-table thead th.right { text-align: ${isRTL ? 'left' : 'right'}; }
    ...
    .invoice-table tbody td.right { text-align: ${isRTL ? 'left' : 'right'}; font-weight: 600; }
    .invoice-table tbody td.center { text-align: center; }
    .invoice-table .discount-row { background: #F0FDF4; }
    .invoice-table .discount-row td { color: #059669; }
    .bottom-section {
      display: grid;
    ...
    .pay-row .value.purple { color: #7C3AED; }
    .totals-box {
      background: linear-gradient(135deg, #0F172A, #1E3A8A);
      border-radius: 8px;
      padding: 10px 14px;
    ...
    .code-section img { height: 28px; width: auto; border-radius: 4px; border: 1px solid #E2E8F0; padding: 2px 6px; background: #FFFFFF; }
    .code-label { font-size: 6px; font-weight: 700; text-transform: uppercase; text-align: center; color: #475569; line-height: 1.2; }
    .code-label span { display: block; font-family: 'Cairo', sans-serif; font-size: 6px; color: #1E3A8A; }
    .ai-message {
      flex: 1;
      text-align: center;
      padding: 4px 12px;
    ...
    .ai-message .ai-label { font-size: 6px; color: #3B82F6; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px; }
    .ai-message p { font-size: 8px; color: #1E3A8A; margin: 0; line-height: 1.4; font-weight: 500; }
    .ai-message p span { display: block; font-family: 'Cairo', sans-serif; font-size: 8px; margin-top: 2px; }
    .footer-info { text-align: center; min-width: 70px; }
    .footer-info p { font-size: 6px; color: #94A3B8; margin: 1px 0; }
    .footer-info .name { font-weight: 700; color: #0F172A; font-size: 7px; }
    @media print {
...
<div class="invoice-container">
  ...
  <div class="body">
    ...
    <div class="store-credit-box">
      <span class="label">💰 ${isRTL ? 'الرصيد المتاح' : 'Available Store Credit'}</span>
    ...
    <table class="invoice-table">
      <thead><tr><th>Description<span>الوصف</span></th><th class="center">Qty<span>الكمية</span></th><th class="right">Unit Price<span>سعر الوحدة</span></th><th class="right">Total<span>الإجمالي</span></th></tr></thead>
      <tbody>
  ...
  <div class="footer">
    <div class="code-section">
      <img src="${barcode}" alt="Barcode" crossorigin="anonymous" style="height:48px;width:auto;min-width:190px;"/>
      <div class="code-label">CHECK-IN<span>تسجيل</span></div>
      <img src="${qr}" alt="QR" crossorigin="anonymous" style="height:55px;width:55px;padding:2px;"/>
  ...
  <div class="linked-section">
    <div class="title">🔄 ${isRTL ? 'الحجز السابق والاسترجاع المرتبط' : 'Previous Booking & Linked Refund'}</div>
    <div class="reissue-block">
      ...
      <div class="reissue-grid">
        ...
        <div class="reissue-fare"><div class="label">Original Fare / الأجرة الأصلية</div><div class="value">${prevFare.toFixed(2)} SAR</div></div>
      </div>
    </div>
    <div class="refund-details">
      <div class="refund-title"><span>🔄 REFUND DETAILS / تفاصيل الاسترجاع</span><span>REF-${inv.linked_inv_id || 'N/A'}</span></div>
      <div class="refund-grid">
        <div class="item"><div class="label">${isRTL ? 'المبلغ المسترد' : 'Refund Amount'}</div><div class="value">${prevRefundCust.toFixed(2)} SAR</div></div>
        <div class="item"><div class="label">${isRTL ? 'تاريخ الاسترجاع' : 'Refund Date'}</div><div class="value">${inv.refund_date || inv.invoice_date || 'N/A'}</div></div>
        <div class="item"><div class="label">${isRTL ? 'السبب' : 'Reason'}</div><div class="value">${prevRefundReason || 'N/A'}</div></div>
      </div>
    </div>
  </div>` : ''}
</div>
...
/* ================================================================
   REFUND INVOICE HTML - MODERN DESIGN (FULL PAGE FILL)
   ================================================================ */
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; padding: 0; }
    ...
    .detail-item { display: flex; flex-direction: column; }
    .detail-item .label { font-size: 7px; color: #991B1B; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
    .detail-item .value { font-size: 9px; color: #7F1D1D; font-weight: 700; }
    .body {
      padding: 16px 24px;
      flex: 1;
      display: flex;
    ...
    .section-title .ar { font-family: 'Cairo', sans-serif; color: #991B1B; font-weight: 600; }
    .info-block {
      padding: 10px 14px;
      background: linear-gradient(135deg, #FEF2F2, #FEE2E2);
      border-radius: 8px;
    ...
    .info-row .value { color: #7F1D1D; font-weight: 600; text-align: ${isRTL ? 'left' : 'right'}; max-width: 65%; word-break: break-word; }
    .refund-summary {
      background: #FFFFFF;
      padding: 12px 16px;
      border-radius: 8px;
    ...
    .code-section { display: flex; align-items: center; gap: 8px; }
    .code-section img { height: 28px; width: auto; border-radius: 4px; border: 1px solid #FECACA; padding: 2px 6px; background: #FFFFFF; }
    .code-label { font-size: 6px; font-weight: 700; text-transform: uppercase; text-align: center; color: #991B1B; line-height: 1.2; }
    .code-label span { display: block; font-family: 'Cairo', sans-serif; font-size: 6px; }
    .ai-message {
      flex: 1;
      text-align: center;
      padding: 4px 12px;
    ...
    .ai-message .ai-label { font-size: 6px; color: #DC2626; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px; }
    .ai-message p { font-size: 8px; color: #7F1D1D; margin: 0; line-height: 1.4; font-weight: 500; }
    .ai-message p span { display: block; font-family: 'Cairo', sans-serif; font-size: 8px; margin-top: 2px; }
    .footer-info { text-align: center; min-width: 70px; }
    .footer-info p { font-size: 6px; color: #94A3B8; margin: 1px 0; }
    .footer-info .name { font-weight: 700; color: #7F1D1D; font-size: 7px; }
    @media print {
      body { background: white; padding: 0; margin: 0; }
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; padding: 0; }
    ...
    @keyframes shimmerBorder { 0%,100% { background-position: 0% 0%; } 50% { background-position: 100% 0%; } }
    .header {
      display: flex;
      justify-content: space-between;
      align-items: center;
    ...
    .header::before { content: '📊'; position: absolute; right: -20px; bottom: -20px; font-size: 120px; opacity: 0.05; transform: rotate(-15deg); }
    .company-text h2 { font-size: 18px; font-weight: 900; color: #FBBF24; margin: 0; }
    .company-text h1 { font-size: 11px; font-weight: 600; color: rgba(255,255,255,0.8); text-transform: uppercase; letter-spacing: 2px; margin: 2px 0 0; }
    .invoice-meta { text-align: ${isRTL ? 'left' : 'right'}; }
    .invoice-meta h3 { font-size: 22px; font-weight: 900; color: #FBBF24; text-transform: uppercase; line-height: 1.1; margin: 0; }
    .invoice-meta p { font-size: 9px; color: rgba(255,255,255,0.8); margin: 2px 0 0; }
    ...
    .detail-item { display: flex; flex-direction: column; }
    .detail-item .label { font-size: 7px; color: #9A3412; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
    .detail-item .value { font-size: 9px; color: #7C2D12; font-weight: 700; }
    .body {
      padding: 16px 24px;
      flex: 1;
      display: flex;
    ...
    .section-title .ar { font-family: 'Cairo', sans-serif; color: #9A3412; font-weight: 600; }
    .info-block {
      padding: 10px 14px;
      background: linear-gradient(135deg, #FFF7ED, #FFEDD5);
      border-radius: 8px;
    ...
    .info-row .value { color: #7C2D12; font-weight: 600; text-align: ${isRTL ? 'left' : 'right'}; }
    .expense-table {
      width: 100%;
      border-collapse: collapse;
      border-radius: 8px;
    ...
    .expense-table thead th span { font-family: 'Cairo', sans-serif; font-size: 8px; opacity: 0.7; display: block; font-weight: 400; }
    .expense-table thead th.right { text-align: ${isRTL ? 'left' : 'right'}; }
    ...
    .expense-table tbody td.right { text-align: ${isRTL ? 'left' : 'right'}; font-weight: 600; }
    .total-box {
      background: linear-gradient(135deg, #7C2D12, #9A3412);
      color: #FFFFFF;
      padding: 14px 18px;
    ...
    .total-box h3 { margin: 0; text-transform: uppercase; font-size: 10px; font-weight: 700; color: #FBBF24; }
    .total-box .amount { font-size: 20px; font-weight: 900; margin: 0; color: #FBBF24; }
    .footer {
      padding: 10px 24px;
      background: linear-gradient(135deg, #FFF7ED, #FFEDD5);
    ...
    .code-section { display: flex; align-items: center; gap: 8px; }
    .code-section img { height: 28px; width: auto; border-radius: 4px; border: 1px solid #FED7AA; padding: 2px 6px; background: #FFFFFF; }
    .code-label { font-size: 6px; font-weight: 700; text-transform: uppercase; text-align: center; color: #7C2D12; line-height: 1.2; }
    .code-label span { display: block; font-family: 'Cairo', sans-serif; font-size: 6px; color: #9A3412; }
    .ai-message {
      flex: 1;
      text-align: center;
      padding: 4px 12px;
    ...
    .ai-message .ai-label { font-size: 6px; color: #9A3412; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px; }
    .ai-message p { font-size: 8px; color: #7C2D12; margin: 0; line-height: 1.4; font-weight: 500; }
    .ai-message p span { display: block; font-family: 'Cairo', sans-serif; font-size: 8px; margin-top: 2px; }
    .footer-info { text-align: center; min-width: 70px; }
    .footer-info p { font-size: 6px; color: #94A3B8; margin: 1px 0; }
    .footer-info .name { font-weight: 700; color: #7C2D12; font-size: 7px; }
    @media print {
      body { background: white; padding: 0; margin: 0; }
...
<div class="expense-container">
  ...
  <div class="body">
    ...
    <table class="expense-table">
      <thead><tr><th>Item / البند<span>الوصف</span></th><th class="right">Qty<span>الكمية</span></th><th class="right">Price<span>السعر</span></th><th class="right">Total<span>المجموع</span></th></tr></thead>
      <tbody>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; padding: 0; }
    ...
    .invoice-meta { text-align: ${isRTL ? 'left' : 'right'}; }
    .invoice-meta h3 { font-size: 22px; font-weight: 900; color: #FBBF24; text-transform: uppercase; line-height: 1.1; margin: 0; }
    .invoice-meta h3 span { font-size: 10px; font-weight: 600; opacity: 0.7; display: block; font-family: 'Cairo', sans-serif; }
    .invoice-meta p { font-size: 9px; color: rgba(255,255,255,0.8); margin: 2px 0 0; }
    .invoice-meta p span { color: #FBBF24; font-weight: 700; }
    .company-details {
      display: grid;
    ...
    .detail-item { display: flex; flex-direction: column; }
    .detail-item .label { font-size: 7px; color: #1E3A8A; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
    .detail-item .value { font-size: 9px; color: #0F172A; font-weight: 700; }
    .body {
      padding: 16px 24px;
      flex: 1;
    ...
    .salary-block.net { border-left: 3px solid #2563EB; background: linear-gradient(135deg, #EFF6FF, #DBEAFE); }
    .salary-block .title { font-size: 8px; font-weight: 700; text-transform: uppercase; color: #0F172A; margin-bottom: 6px; padding-bottom: 4px; border-bottom: 1px solid #E2E8F0; }
    .salary-block .title .ar { font-family: 'Cairo', sans-serif; color: #64748B; font-weight: 600; font-size: 7px; }
    .salary-row {
      display: flex;
      justify-content: space-between;
      font-size: 9px;
    ...
    .salary-row .value { color: #0F172A; font-weight: 600; text-align: ${isRTL ? 'left' : 'right'}; }
    ...
    .code-section { display: flex; align-items: center; gap: 8px; }
    .code-section img { height: 28px; width: auto; border-radius: 4px; border: 1px solid #BFDBFE; padding: 2px 6px; background: #FFFFFF; }
    .code-label { font-size: 6px; font-weight: 700; text-transform: uppercase; text-align: center; color: #1E3A8A; line-height: 1.2; }
    .code-label span { display: block; font-family: 'Cairo', sans-serif; font-size: 6px; color: #3B82F6; }
    .ai-message {
      flex: 1;
      text-align: center;
      padding: 4px 12px;
...
export const getContractHTML = (settings, corpName, date, isOffer = false, serviceType = 'Flight Tickets', markup = '10', terms = '') => {
  ...
  const isRTL = false; // contracts usually in English
  const dir = 'ltr';
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; padding: 0; }
    ...
    .header::before { content: '📄'; position: absolute; right: -20px; bottom: -20px; font-size: 120px; opacity: 0.05; transform: rotate(-15deg); }
    .logo-area { display: flex; align-items: center; gap: 16px; z-index: 1; }
    .logo-wrapper {
      width: 80px;
      height: 80px;
      border-radius: 14px;
    ...
    .logo-wrapper img { width: 100%; height: 100%; object-fit: cover; }
    .logo-wrapper .logo-placeholder { font-size: 36px; }
    .company-info h1 { margin: 0; font-size: 20px; font-weight: 900; color: #FBBF24; }
    .company-info h2 { margin: 2px 0 0; font-size: 11px; font-weight: 600; color: rgba(255,255,255,0.8); text-transform: uppercase; letter-spacing: 2px; }
    .company-info p { margin: 5px 0 0; font-size: 9px; color: rgba(255,255,255,0.6); line-height: 1.6; }
    .doc-badge {
      background: rgba(251,191,36,0.15);
      color: #FBBF24;
      padding: 10px 24px;
    ...
    .doc-badge span { display: block; font-size: 9px; font-weight: 600; opacity: 0.7; margin-top: 2px; }
    .body {
      padding: 30px 32px;
      flex: 1;
      display: flex;
    ...
    .title { font-size: 22px; font-weight: 800; color: #1E3A8A; margin: 0 0 20px; padding-bottom: 12px; border-bottom: 3px solid #E2E8F0; }
    .section { margin-bottom: 24px; }
    .section h3 { font-size: 14px; font-weight: 700; color: #1E3A8A; margin: 0 0 12px; border-left: 4px solid #3B82F6; padding-left: 12px; }
    ...
    .info-item .label { font-size: 9px; color: #64748B; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
    .info-item .value { font-size: 13px; color: #0F172A; font-weight: 700; margin-top: 2px; }
    .terms-box { background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 20px 24px; margin-top: 16px; }
    ...
    .term-num { color: #3B82F6; font-weight: 700; font-size: 13px; min-width: 24px; }
    ...
    .sig-name { font-size: 13px; font-weight: 700; color: #1E3A8A; margin-top: 8px; }
    ...
    .code-section { display: flex; align-items: center; gap: 10px; }
    .code-section img { height: 30px; width: auto; border-radius: 4px; border: 1px solid #E2E8F0; padding: 2px 6px; background: #FFFFFF; }
    .code-label { font-size: 6px; font-weight: 700; text-transform: uppercase; text-align: center; color: #475569; line-height: 1.2; }
    .code-label span { display: block; font-size: 6px; color: #1E3A8A; }
    @media print {
      body { background: white; padding: 0; margin: 0; }
  ...
  <div class="body">
    ...
    <div class="section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item"><div class="label">Validity / الصلاحية</div><div class="value">30 Days from issue date</div></div>
      </div>
    </div>
    <div class="section">
      ...
      <div class="info-grid">
        ...
        <div class="info-item"><div class="label">Payment Terms / شروط الدفع</div><div class="value">100% Advance Required</div></div>
      </div>
    </div>
    <div class="terms-box">
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; padding: 0; }
    ...
    .header::before { content: '⚠️'; position: absolute; right: -20px; bottom: -20px; font-size: 120px; opacity: 0.05; transform: rotate(-15deg); }
    .logo-area { display: flex; align-items: center; gap: 16px; z-index: 1; }
    .logo-wrapper {
      width: 60px;
      height: 60px;
    ...
    .logo-wrapper img { width: 100%; height: 100%; object-fit: cover; }
    .logo-wrapper .logo-placeholder { font-size: 28px; }
    .company-info h2 { margin: 0; font-size: 18px; font-weight: 900; color: #FBBF24; }
    .company-info h1 { margin: 2px 0 0; font-size: 10px; font-weight: 600; color: rgba(255,255,255,0.8); text-transform: uppercase; letter-spacing: 2px; }
    .company-info p { margin: 5px 0 0; font-size: 8px; color: rgba(255,255,255,0.6); line-height: 1.5; }
    .doc-badge {
      background: rgba(251,191,36,0.15);
      color: #FBBF24;
      padding: 8px 20px;
    ...
    .doc-badge span { display: block; font-size: 8px; font-weight: 600; opacity: 0.7; margin-top: 2px; }
    .body {
      padding: 24px 32px;
      flex: 1;
      display: flex;
    ...
    .title { font-size: 20px; font-weight: 800; color: #7F1D1D; margin: 0 0 8px; padding-bottom: 10px; border-bottom: 3px solid #FECACA; }
    .section { margin-bottom: 16px; }
    .section h3 { font-size: 14px; font-weight: 700; color: #7F1D1D; margin: 0 0 10px; border-left: 4px solid #DC2626; padding-left: 12px; }
    .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    .info-item { background: #FFF5F5; padding: 10px 14px; border-radius: 8px; border: 1px solid #FECACA; }
    .info-item .label { font-size: 8px; color: #991B1B; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
    .info-item .value { font-size: 11px; color: #7F1D1D; font-weight: 700; margin-top: 2px; }
    .danger-box { background: #FEF2F2; border: 2px solid #FECACA; border-radius: 8px; padding: 16px 20px; }
    .danger-title { font-size: 14px; font-weight: 700; color: #DC2626; margin: 0 0 10px; display: flex; justify-content: space-between; align-items: center; background: #FEF2F2; padding: 8px 12px; border-radius: 6px; }
    ...
    .danger-row .value { font-weight: 700; }
    .loss-amount { font-size: 28px; font-weight: 900; color: #DC2626; text-align: center; padding: 12px 0; background: #FEF2F2; border-radius: 8px; border: 2px dashed #FECACA; }
    .ai-box { background: #FFFBEB; border: 1px solid #FDE68A; border-radius: 8px; padding: 12px 16px; text-align: center; }
    .ai-box .ai-label { font-size: 7px; color: #991B1B; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }
    .ai-box p { font-size: 9px; color: #78350F; margin: 0; line-height: 1.5; }
    .ai-box p span { display: block; font-family: 'Cairo', sans-serif; font-size: 9px; margin-top: 2px; }
    .footer {
      padding: 12px 32px;
...
<div class="mistake-container">
  ...
  <div class="body">
    ...
    <div class="section">
      ...
      <div class="info-grid">
        <div class="info-item"><div class="label">Date / التاريخ</div><div class="value">${m.date || 'N/A'}</div></div>
        <div class="info-item"><div class="label">Employee / الموظف</div><div class="value">${m.employees?.name || 'N/A'}</div></div>
        <div class="info-item"><div class="label">Old Ticket No / التذكرة القديمة</div><div class="value">${m.old_ticket_no || 'N/A'}</div></div>
        <div class="info-item"><div class="label">New Ticket No / التذكرة الجديدة</div><div class="value">${m.new_ticket_no || 'N/A'}</div></div>
        <div class="info-item" style="grid-column: span 2;"><div class="label">Reason / السبب</div><div class="value">${m.reason || 'N/A'}</div></div>
      </div>
    </div>
    <div class="danger-box">
      <div class="danger-title">⚠️ FINANCIAL IMPACT</div>
      <div class="danger-row"><span>Ticket Price / سعر التذكرة</span><span class="value">${parseFloat(m.old_sell_price || 0).toFixed(2)} SAR</span></div>
      <div class="danger-row"><span>Airline Fees / رسوم الخطوط</span><span class="value">- ${((m.old_sell_price || 0) - (m.refund_company || 0)).toFixed(2)} SAR</span></div>
      <div class="danger-row"><span>Customer Refund / استرجاع العميل</span><span class="value">${(m.refund_customer || 0).toFixed(2)} SAR</span></div>
    </div>
    <div class="loss-amount">LOSS: ${(m.loss_amount || 0).toFixed(2)} SAR</div>
    <div class="section">
      <h3>Deduction Details / تفاصيل الخصم</h3>
      <div class="info-grid">
        <div class="info-item"><div class="label">Loss Amount / مبلغ الخسارة</div><div class="value" style="color:#DC2626;">${(m.loss_amount || 0).toFixed(2)} SAR</div></div>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; margin: 0; padding: 0; }
    ...
    .section-title .ar { font-family: 'Cairo', sans-serif; color: #64748B; font-weight: 600; }
    .info-block { padding: 10px 14px; background: #F8FAFC; border-radius: 8px; border-left: 3px solid #2563EB; }
    .info-row {
      display: flex;
      justify-content: space-between;
      font-size: 9px;
...
<div class="recharge-container">
  ...
  <div class="body">
    ...
    <div class="total-box">
      <h3>AMOUNT / المبلغ</h3>
      <p class="amount">${amount.toFixed(2)} SAR</p>
    </div>
  </div>
  <div class="footer">
    <p>© ${new Date().getFullYear()} ${st.company_name_en || 'SUEUD AL TAAYIRA'}</p>
    <p style="font-size:7px;color:#94A3B8">Computer-generated - Valid without signature</p>
  </div>
</div>
</body>
</html>`;
