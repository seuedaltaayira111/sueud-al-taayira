...
<div class="ft">
  <div class="code-box">
    <div style="text-align:center"><img src="${barcode}" alt="Barcode" class="barcode-img" crossorigin="anonymous"/><div class="code-label checkin">CHECK-IN BARCODE<span>باركود تسجيل الدخول</span></div></div>
    <div style="text-align:center"><img src="${qr}" alt="QR Code" class="qr-img" crossorigin="anonymous"/><div class="code-label download">DOWNLOAD QR<span>تحميل QR</span></div></div>
  </div>
  <div class="ft-divider"></div>
  <div class="ai-msg"><div class="ai-label">🤖 AI MESSAGE</div><p>${aiMsg}<span>${lang === 'ar' ? '✈️ رحلة سعيدة!' : 'Safe flight!'}</span></p></div>
  <div class="ft-divider"></div>
  <div class="ft-info"><p style="font-weight:700;color:#0f172a;font-size:8px">${st.company_name_en || 'SUEUD AL TAAYIRA'}</p><p>${st.phone || ''}</p><p style="font-family:'Cairo';font-size:8px">${st.company_name_ar || ''}</p><p style="color:#cbd5e1;margin-top:3px">${st.vat_no ? 'VAT: ' + st.vat_no : ''}</p></div>
</div></div></body></html>`;
};

/* ═══════════════════════════════════════════════════════════════
   REFUND HTML GENERATOR (FULL BEAUTIFUL)
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   EXPENSE HTML GENERATOR (FULL BEAUTIFUL)
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   SALARY SLIP HTML GENERATOR (FULL BEAUTIFUL)
   ═══════════════════════════════════════════════════════════════ */
...
.comp-det .cv{font-size:9px;color:#065f46;font-weight:600}
.body{padding:12px 18px;flex:1;display:flex;flex-direction:column;gap:10px}
...
.row{display:flex;justify-content:space-between;font-size:9px;padding:4px 0;border-bottom:1px solid #d1fae5}
.row:last-child{border:none}.row .l{color:#065f46;font-weight:500}.row .v{color:#064e3b;font-weight:600}
table{width:100%;border-collapse:collapse;border-radius:6px;overflow:hidden;border:2px solid #e2e8f0}
...
.ai-msg{text-align:center;flex:1;padding:6px 14px;background:linear-gradient(135deg,#ecfdf5,#d1fae5);border-radius:8px;border:1px solid #a7f3d0}
...
<div class="hdr">
  ...
  <div class="im">
    ...
    <div class="mr"><span class="l">Pay Date / تاريخ</span><span class="v">${pay.payment_date || ''}</span></div>
  </div>
</div>
<div class="comp-det">
  <div class="ci"><span class="cl">VAT No / رقم ضريبي</span><span class="cv">${st.vat_no || 'N/A'}</span></div>
...
<div class="body">
  <div class="ib">
    ...
    <div class="row"><span class="l">Name / الاسم</span><span class="v">${emp.name || 'N/A'}</span></div>
    <div class="row"><span class="l">Job Title / المسمى</span><span class="v">${emp.job_title || 'N/A'}</span></div>
    <div class="row"><span class="l">ID / رقم الهوية</span><span class="v">${emp.national_id || 'N/A'}</span></div>
  </div>
  <table>
    <thead><tr>
      <th style="text-align:${textAlign}">Description / الوصف</th>
      <th class="r">Earnings (SAR)<span>الإيرادات</span></th>
      <th class="r">Deductions (SAR)<span>الخصومات</span></th>
    </tr></thead>
    <tbody>
      <tr><td>${isRTL ? 'الراتب الأساسي' : 'Base Salary'}</td><td class="r" style="color:#059669">${base.toFixed(2)}</td><td class="r">-</td></tr>
      ${commission > 0 ? `<tr><td>${isRTL ? 'عمولة' : 'Commission'}</td><td class="r" style="color:#059669">${commission.toFixed(2)}</td><td class="r">-</td></tr>` : ''}
      ${overtime > 0 ? `<tr><td>${isRTL ? 'وقت إضافي' : 'Overtime'}</td><td class="r" style="color:#059669">${overtime.toFixed(2)}</td><td class="r">-</td></tr>` : ''}
      ${gift > 0 ? `<tr><td>${isRTL ? 'هدية / مكافأة' : 'Gift / Bonus'}</td><td class="r" style="color:#059669">${gift.toFixed(2)}</td><td class="r">-</td></tr>` : ''}
      ${advDed > 0 ? `<tr><td>${isRTL ? 'خصم سلفة' : 'Advance Deduction'}</td><td class="r">-</td><td class="r" style="color:#dc2626">- ${advDed.toFixed(2)}</td></tr>` : ''}
    </tbody>
  </table>
</div>
<div class="totals">
  <h3>NET PAY / صافي الراتب</h3>
  <p class="amt">${net.toFixed(2)} SAR</p>
</div>
<div class="ft">
  <div class="code-box">
    ...
    <div style="text-align:center"><img src="${qr}" alt="QR Code" class="qr-img" crossorigin="anonymous"/><div class="code-label">VERIFY QR<span>تحقق QR</span></div></div>
  </div>
  <div class="ft-divider"></div>
...
/* ═══════════════════════════════════════════════════════════════
   CONTRACT HTML GENERATOR (FULL BEAUTIFUL)
   ═══════════════════════════════════════════════════════════════ */
export const getContractHTML = (data, s, lang = 'en') => {
  const st = s || {};
  const isRTL = lang === 'ar';
  const dir = isRTL ? 'rtl' : 'ltr';
  const textAlign = isRTL ? 'right' : 'left';
  return `<!DOCTYPE html><html lang="${lang}" dir="${dir}"><head><meta charset="UTF-8"><title>Contract</title>
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800&family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<style>
@page{size:A4 portrait;margin:0}
*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0;padding:0}
body{font-family:'Inter','Cairo',sans-serif;background:#fff;color:#1e293b;font-size:11px;line-height:1.6;padding:40px}
.hdr{text-align:center;border-bottom:3px solid #0c1d3a;padding-bottom:20px;margin-bottom:30px}
.hdr h1{color:#0c1d3a;font-size:24px;margin:0}
.hdr h2{color:#f59e0b;font-size:16px;margin:5px 0 0}
.body p{margin-bottom:15px;line-height:1.8}
.body .highlight{background:#eff6ff;padding:15px;border-radius:8px;border-left:4px solid #2563eb;margin:20px 0}
.body .grid{display:grid;grid-template-columns:1fr 1fr;gap:15px;margin:20px 0}
.body .grid-item{background:#f8fafc;padding:10px;border-radius:6px;border:1px solid #e2e8f0}
.body .grid-item strong{color:#1e40af}
.sig{display:flex;justify-content:space-between;margin-top:100px}
.sig div{text-align:center;width:45%}
.sig hr{border:1px solid #000;margin-top:60px}
...
<div class="body">
  <h3 style="color:#0c1d3a; border-bottom:2px solid #e2e8f0; padding-bottom:5px;">Corporate Agreement / اتفاقية شركات</h3>
  <div class="grid">
    <div class="grid-item"><strong>Company / الشركة:</strong><br/>${data.corpName || 'N/A'}</div>
    <div class="grid-item"><strong>Service Type / نوع الخدمة:</strong><br/>${data.type || 'Flights'}</div>
    <div class="grid-item"><strong>Markup / هامش الربح:</strong><br/>${data.markup || '10'}%</div>
    <div class="grid-item"><strong>Date / التاريخ:</strong><br/>${new Date().toLocaleDateString()}</div>
  </div>
  <div class="highlight">
    <strong>Terms & Conditions / الشروط والأحكام</strong><br/><br/>
    <div style="white-space:pre-wrap;">${data.terms || 'No terms provided.'}</div>
  </div>
</div>
<div class="sig">
  <div><strong>Authorized Signature / التوقيع المعتمد</strong><hr/></div>
  <div><strong>Client Signature / توقيع العميل</strong><hr/></div>
</div>
</body></html>`;
};

/* ═══════════════════════════════════════════════════════════════
   MISTAKE HTML GENERATOR (FULL BEAUTIFUL)
   ═══════════════════════════════════════════════════════════════ */
export const getMistakeHTML = (data, s, lang = 'en') => {
  const st = s || {};
  const isRTL = lang === 'ar';
  const dir = isRTL ? 'rtl' : 'ltr';
  const textAlign = isRTL ? 'right' : 'left';
  const textAlignR = isRTL ? 'left' : 'right';
  return `<!DOCTYPE html><html lang="${lang}" dir="${dir}"><head><meta charset="UTF-8"><title>Staff Mistake</title>
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800&family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<style>
@page{size:A4 portrait;margin:0}
*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0;padding:0}
body{font-family:'Inter','Cairo',sans-serif;background:#fff;color:#1e293b;font-size:9px;line-height:1.4}
.inv{width:210mm;min-height:148mm;margin:auto;border:2px solid #dc2626;overflow:hidden}
.hdr{background:linear-gradient(135deg,#991b1b,#dc2626);color:#fff;padding:20px;display:flex;justify-content:space-between;align-items:center}
.title h1{font-size:18px;color:#fbbf24;margin:0}
.meta h2{font-size:20px;color:#fbbf24;margin:0;text-align:${textAlignR}}
.body{padding:20px}
.row{display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #fecaca;font-size:10px}
.total-box{background:#fef2f2;padding:15px;border-radius:8px;margin-top:20px;display:flex;justify-content:space-between;font-size:16px;font-weight:bold;color:#991b1b;border:2px solid #fca5a5}
.ft{margin-top:20px;text-align:center;font-size:9px;color:#64748b;background:#fff5f5;padding:10px;border-top:1px solid #fecaca}
@media print{body{background:#fff;padding:0;margin:0}.inv{border:none}}
</style></head><body>
<div class="inv">
  <div class="hdr">
    <div class="title"><h1>${st.company_name_ar || 'صعود الطائرة'}</h1><p style="font-size:9px;opacity:0.8;margin:5px 0 0">${st.company_name_en || 'SUEUD AL TAAYIRA'}</p></div>
    <div class="meta"><h2>⚠️ MISTAKE / خطأ</h2><p style="font-size:9px;margin:4px 0 0">Date: ${data.date || ''}</p></div>
  </div>
  <div class="body">
    <div class="row"><span>Employee / الموظف</span><span style="font-weight:600">${data.employees?.name || 'N/A'}</span></div>
    <div class="row"><span>Old Ticket / التذكرة القديمة</span><span style="font-weight:600">${data.old_ticket_no || 'N/A'}</span></div>
    <div class="row"><span>New Ticket / التذكرة الجديدة</span><span style="font-weight:600">${data.new_ticket_no || 'N/A'}</span></div>
    <div class="row"><span>Deduct from Salary / خصم من الراتب</span><span style="font-weight:600">${data.deduct_from_salary ? 'Yes / نعم' : 'No / لا'}</span></div>
    <div class="total-box"><span>LOSS AMOUNT / مبلغ الخسارة</span><span>${(parseFloat(data.loss_amount) || 0).toFixed(2)} SAR</span></div>
  </div>
  <div class="ft"><p>Computer Generated Document / مستند آلي صالح بدون توقيع</p></div>
</div>
</body></html>`;
};
