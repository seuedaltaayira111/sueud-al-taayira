...
<div class="body">
  <div class="dg">
    <div class="ib earn"><div class="sec-title">EARNINGS / الإيرادات</div>
      ...
      <div class="row"><span class="l">Commission / العمولة</span><span class="v">${(pay.commission_amount || 0).toFixed(2)} SAR</span></div>
...
export const getContractHTML = (settings, corpName, date, isOffer = false, serviceType = 'Flight Tickets', markup = '10', terms = '') => {
  ...
  const aiMsg = getAIMessage({ id: Date.now() }, 'en');
  const isRTL = false;
  const dir = isRTL ? 'rtl' : 'ltr';
...
<style>
@page{size:A4 portrait;margin:20mm}*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0;padding:0}
body{font-family:'Inter','Cairo',sans-serif;background:#fff;color:#1e293b;font-size:11px;line-height:1.5}
.doc{max-width:210mm;margin:0 auto;background:#fff;border:2px solid #1e3a8a;border-radius:12px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.1)}
...
.header{padding:30px;background:linear-gradient(135deg,#1e3a8a,#0f172a);color:#fff;display:flex;justify-content:space-between;align-items:center;gap:20px}
...
.signature-area{display:flex;justify-content:space-between;margin-top:40px;padding-top:20px;border-top:2px solid #e2e8f0}
.sig-block{text-align:center;flex:1}
.sig-line{width:200px;height:1px;background:#1e3a8a;margin:0 auto 8px 0}
.sig-name{font-size:13px;font-weight:600;color:#1e3a8a;margin-top:8px}
.sig-role{font-size:11px;color:#64748b}
.footer{padding:20px 30px;background:#f8fafc;border-top:2px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center;font-size:10px;color:#64748b}
...
<div class="footer">
  ...
  <p style="font-size:9px;color:#94a3b8">Computer-generated document - Valid without physical signature as per Saudi e-invoicing regulations (Fatoorah)</p>
</div>
</div>
</div></body></html>`;
};
