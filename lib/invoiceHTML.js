...
/* ═══════════════════════════════════════════════════════════════
   SALARY SLIP HTML GENERATOR
   Signature: (pay, settings, lang)
   ═══════════════════════════════════════════════════════════════ */
...
<div class="body">
  ...
  <table>
    ...
    <tbody>
      ...
      <tr><td>Overtime / إضافي</td><td class="r">${overtime.toFixed(2)}</td><td></td><td class="r"></td></tr>
...
/* ═══════════════════════════════════════════════════════════════
   CONTRACT / OFFER HTML GENERATOR
   Signature: (settings, corpName, date, isOffer, type, markup, terms)
   ═══════════════════════════════════════════════════════════════ */
export const getContractHTML = (st, corpName, contractDate, isOffer, contractType, markup, terms) => {
  st = st || {};
  const isRTL = false;
  const dir = 'ltr';
  const textAlign = 'left';
  const textAlignR = 'right';
  const docType = isOffer ? 'CORPORATE OFFER' : 'CORPORATE CONTRACT';
  const docTypeAr = isOffer ? 'عرض شركات' : 'عقد شركات';
  const docNo = `CO-${Date.now().toString(36).toUpperCase()}`;
  const docURL = `https://sueud-al-taayira.vercel.app/contract/${docNo}`;
  const qr = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(docURL)}&color=1e3a5f`;
  const barcodeData = `${isOffer ? 'OFFER' : 'CONTRACT'}-${docNo}-${contractDate || ''}`;
  const barcode = `https://bwipjs-api.metafloor.com/?bcid=code128&text=${encodeURIComponent(barcodeData)}&scale=2&height=40&barcolor=1e3a5f&backgroundcolor=ffffff&includetext=false`;
  const aiMsg = isOffer ? '🌟 Thank you for considering our corporate services!' : '📋 We look forward to a successful partnership!';
  const markupPct = parseFloat(markup) || 10;
  const bgColor = isOffer ? '#fef3c7' : '#eff6ff';
  const borderColor = isOffer ? '#f59e0b' : '#2563eb';
  const headerBg = isOffer ? 'linear-gradient(135deg,#92400e,#b45309)' : 'linear-gradient(135deg,#1e3a5f,#1e40af)';

  return `<!DOCTYPE html><html lang="en" dir="ltr"><head><meta charset="UTF-8"><title>${docType}</title>
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800&family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<style>
@page{size:A4 portrait;margin:0}
*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0;padding:0}
body{font-family:'Inter','Cairo',sans-serif;background:#fff;color:#1e293b;font-size:10px;line-height:1.5}
.inv{width:210mm;min-height:297mm;margin:auto;border:2px solid ${borderColor};display:flex;flex-direction:column;position:relative;overflow:hidden}
.inv::before{content:'';position:absolute;top:0;left:0;right:0;height:6px;background:${headerBg}}
.inv::after{content:'';position:absolute;bottom:0;left:0;right:0;height:6px;background:${headerBg}}
.hdr{display:flex;justify-content:space-between;padding:15px 20px;background:${headerBg};color:#fff;gap:14px;margin-top:6px;align-items:center}
.cblk{display:flex;gap:12px;flex:1;align-items:center}
.logo{width:60px;height:60px;object-fit:cover;border-radius:8px;background:rgba(255,255,255,0.1);padding:3px;border:2px solid rgba(251,191,36,0.4)}
.ct h2{font-size:16px;font-weight:800;color:#fbbf24;margin:0;font-family:'Cairo'}
.ct h1{font-size:10px;font-weight:600;color:rgba(255,255,255,0.85);text-transform:uppercase;letter-spacing:1.5px;margin:3px 0 0}
.ct p{font-size:9px;color:rgba(255,255,255,0.6);line-height:1.7;margin:4px 0 0}
.im{min-width:180px;background:rgba(255,255,255,0.06);padding:15px;border-radius:10px;border:1px solid rgba(255,255,255,0.12);text-align:right}
.im h3{font-size:22px;font-weight:800;color:#fbbf24;text-transform:uppercase;line-height:1;margin:0}
.im h3 span{font-size:10px;font-family:'Cairo';display:block;margin:3px 0 0;color:rgba(255,255,255,0.8)}
.mr{display:flex;justify-content:space-between;margin-top:4px;font-size:9px;border-bottom:1px dashed rgba(255,255,255,0.15);padding-bottom:3px}
.mr .l{color:rgba(255,255,255,0.6)}.mr .v{color:#fbbf24;font-weight:700}
.comp-det{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;padding:10px 20px;background:${bgColor};border-bottom:2px solid ${borderColor}33}
.comp-det .ci{display:flex;flex-direction:column;gap:1px}
.comp-det .cl{font-size:8px;color:${borderColor};font-weight:600;text-transform:uppercase;letter-spacing:0.3px}
.comp-det .cv{font-size:10px;color:${borderColor};font-weight:600}
.body{padding:15px 20px;flex:1;display:flex;flex-direction:column;gap:12px}
.sec-title{font-size:10px;font-weight:700;text-transform:uppercase;color:${borderColor};margin-bottom:8px;border-bottom:2px solid ${borderColor};padding-bottom:5px;letter-spacing:0.5px}
.ib{padding:12px;background:${bgColor};border-radius:8px;border:1px solid ${borderColor}33;border-left:4px solid ${borderColor}}
.ir{display:flex;justify-content:space-between;font-size:10px;padding:4px 0;border-bottom:1px solid ${borderColor}22}
.ir:last-child{border:none}.ir .l{color:#475569;font-weight:500}.ir .v{color:#1e293b;font-weight:600;text-align:right;max-width:65%}
.service-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}
.service-card{padding:12px;background:#fff;border-radius:8px;border:2px solid ${borderColor}22;text-align:center}
.service-card .icon{font-size:24px;margin-bottom:5px}
.service-card .name{font-size:10px;font-weight:700;color:${borderColor};text-transform:uppercase}
.service-card .desc{font-size:8px;color:#64748b;margin-top:3px}
.markup-box{background:linear-gradient(135deg,${borderColor},${borderColor}cc);color:#fff;padding:20px;border-radius:10px;text-align:center;margin:10px 0}
.markup-box .label{font-size:10px;text-transform:uppercase;letter-spacing:1px;opacity:0.8}
.markup-box .value{font-size:36px;font-weight:800;margin:5px 0;color:#fbbf24}
.terms-box{padding:15px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0}
.terms-box h4{font-size:10px;color:${borderColor};margin:0 0 8px;font-weight:700;text-transform:uppercase}
.terms-box p{font-size:9px;color:#475569;line-height:1.8;margin:0;white-space:pre-wrap}
.validity-box{padding:10px 15px;background:#fef2f2;border:2px solid #fca5a5;border-radius:8px;display:flex;align-items:center;gap:10px}
.validity-box .icon{font-size:20px}
.validity-box .text{font-size:9px;color:#991b1b;line-height:1.5}
.ft{padding:12px 20px;background:${bgColor};display:flex;justify-content:space-between;align-items:center;border-top:2px solid ${borderColor}33;gap:10px;margin-top:auto;min-height:70px}
.code-box{display:flex;align-items:center;gap:10px}
.barcode-img{height:40px;width:auto;min-width:160px;border:2px solid ${borderColor}33;padding:2px 5px;background:#fff;border-radius:4px}
.qr-img{height:50px;width:50px;border:2px solid ${borderColor}33;padding:2px;background:#fff;border-radius:5px}
.code-label{font-size:8px;font-weight:700;text-transform:uppercase;text-align:center;color:${borderColor}}
.code-label span{font-family:'Cairo';display:block;font-size:8px}
.ft-divider{width:2px;background:${borderColor}33;align-self:stretch;min-height:50px}
.ai-msg{text-align:center;flex:1;padding:8px 16px;background:#fff;border-radius:8px;border:1px solid ${borderColor}33}
.ai-msg p{font-size:10px;color:${borderColor};margin:0;line-height:1.5;font-weight:500}
.ai-label{font-size:8px;color:${borderColor}88;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:3px}
.ft-info{width:100px;text-align:center}
.ft-info p{font-size:8px;color:#475569;margin:2px 0}
.sig-area{display:flex;justify-content:space-between;padding:25px 20px 15px;margin-top:auto}
.sig-box{text-align:center;width:40%}
.sig-line{border-top:2px solid ${borderColor};margin-top:45px;padding-top:6px;font-size:9px;color:${borderColor};font-weight:600}
@media print{body{background:#fff;padding:0;margin:0}.inv{border:none}}
</style></head><body>
<div class="inv">
<div class="hdr">
  <div class="cblk">
    ${st.logo_url ? `<img src="${st.logo_url}" crossorigin="anonymous" class="logo"/>` : '<div class="logo" style="display:flex;align-items:center;justify-content:center;font-size:26px">📋</div>'}
    <div class="ct">
      <h2>${st.company_name_ar || 'صعود الطائرة للسفر والسياحة'}</h2>
      <h1>${st.company_name_en || 'SUEUD AL TAAYIRA TRAVEL & TOURISM'}</h1>
      <p>${st.address_ar || 'Address'} ${st.phone ? '| ' + st.phone : ''} ${st.website ? '| ' + st.website : ''}</p>
  ...
  <div class="im">
    <h3>${isOffer ? '🌟' : '📋'} ${docType}<span>${docTypeAr}</span></h3>
    <div class="mr"><span class="l">Ref / مرجع</span><span class="v">${docNo}</span></div>
    <div class="mr"><span class="l">Date / التاريخ</span><span class="v">${contractDate || new Date().toISOString().split('T')[0]}</span></div>
    <div class="mr"><span class="l">Status</span><span class="v" style="color:#34d399">ACTIVE</span></div>
  </div>
</div>
<div class="comp-det">
  <div class="ci"><span class="cl">VAT No / رقم ضريبي</span><span class="cv">${st.vat_no || 'N/A'}</span></div>
...
<div class="body">
  <div class="ib">
    <div class="sec-title">PARTIES / الأطراف</div>
    <div class="ir"><span class="l">Company / الشركة</span><span class="v">${st.company_name_en || 'SUEUD AL TAAYIRA'}</span></div>
    <div class="ir"><span class="l">Corporate Client / العميل الشركات</span><span class="v" style="color:${borderColor};font-weight:700">${corpName || 'N/A'}</span></div>
    <div class="ir"><span class="l">Service Type / نوع الخدمة</span><span class="v">${contractType || 'Flight Tickets'}</span></div>
  </div>
  <div class="markup-box">
    <div class="label">AGREED MARKUP / هامش الربح المتفق عليه</div>
    <div class="value">${markupPct}%</div>
    <div class="label">Applied on all ${contractType || 'services'}</div>
  </div>
  <div class="service-grid">
    <div class="service-card"><div class="icon">✈️</div><div class="name">Flight Tickets</div><div class="desc">Domestic & International</div></div>
    <div class="service-card"><div class="icon">🏨</div><div class="name">Hotel Bookings</div><div class="desc">Worldwide Properties</div></div>
    <div class="service-card"><div class="icon">👥</div><div class="name">Visa Services</div><div class="desc">Tourist, Visit, Umrah</div></div>
    <div class="service-card"><div class="icon">🌍</div><div class="name">Tour Packages</div><div class="desc">Custom & Fixed Packages</div></div>
  </div>
  ${terms ? `<div class="terms-box"><h4>TERMS & CONDITIONS / الشروط والأحكام</h4><p>${terms}</p></div>` : `<div class="terms-box"><h4>TERMS & CONDITIONS / الشروط والأحكام</h4><p>1. All bookings subject to airline/hotel terms and conditions.
2. Prices are subject to availability and may change without prior notice.
3. Payment terms: Net 30 days from invoice date unless otherwise agreed.
4. Cancellation fees apply as per airline/hotel policy.
5. This ${isOffer ? 'offer' : 'contract'} is valid for 90 days from the date of issue.
6. Any disputes shall be resolved under Saudi Arabian law.
7. Markup of ${markupPct}% applies to all services mentioned above.
8. Invoices will be generated with detailed breakdown of costs and markup.</p></div>`}
  <div class="validity-box">
    <div class="icon">⏰</div>
    <div class="text"><strong>Validity:</strong> This ${isOffer ? 'offer' : 'contract'} is valid for 90 days from the date of issue unless terminated earlier by either party with 30 days written notice.</div>
  </div>
</div>
<div class="sig-area">
  <div class="sig-box"><div class="sig-line">Authorized Signature - Company / توقيع الشركة</div></div>
  <div class="sig-box"><div class="sig-line">Authorized Signature - Corporate / توقيع العميل</div></div>
</div>
<div class="ft">
  <div class="code-box">
    <div style="text-align:center"><img src="${barcode}" alt="Barcode" class="barcode-img" crossorigin="anonymous"/><div class="code-label">${isOffer ? 'OFFER' : 'CONTRACT'} BARCODE</div></div>
    <div style="text-align:center"><img src="${qr}" alt="QR Code" class="qr-img" crossorigin="anonymous"/><div class="code-label">VERIFY QR</div></div>
  </div>
  <div class="ft-divider"></div>
  <div class="ai-msg"><div class="ai-label">🤖 AI MESSAGE</div><p>${aiMsg}</p></div>
  <div class="ft-divider"></div>
  <div class="ft-info"><p style="font-weight:700;color:${borderColor}">${st.company_name_en || ''}</p><p>${st.phone || ''}</p><p style="font-family:'Cairo'">${st.company_name_ar || ''}</p><p style="color:#94a3b8;margin-top:5px;font-size:7px">${st.vat_no ? 'VAT: ' + st.vat_no : ''}</p></div>
</div></div></body></html>`;
};

/* ═══════════════════════════════════════════════════════════════
   STAFF MISTAKE / LOSS HTML GENERATOR
   Signature: (mistake, settings, lang)
   ═══════════════════════════════════════════════════════════════ */
export const getMistakeHTML = (mistake, s, lang = 'en') => {
  const st = s || {};
  const mno = mistake.id ? mistake.id.substring(0, 8).toUpperCase() : 'N/A';
  const mistakeURL = `https://sueud-al-taayira.vercel.app/mistake/${mno}`;
  const qr = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(mistakeURL)}&color=7f1d1d`;
  const barcodeData = `MISTAKE-${mno}-${mistake.date || ''}`;
  const barcode = `https://bwipjs-api.metafloor.com/?bcid=code128&text=${encodeURIComponent(barcodeData)}&scale=2&height=40&barcolor=7f1d1d&backgroundcolor=ffffff&includetext=false`;
  ...
  const textAlign = isRTL ? 'right' : 'left';
  const textAlignR = isRTL ? 'left' : 'right';
  const lossAmt = parseFloat(mistake.loss_amount) || 0;
  const paidByEmp = mistake.paid_by_employee;

  return `<!DOCTYPE html><html lang="${lang}" dir="${dir}"><head><meta charset="UTF-8"><title>Mistake Report ${mno}</title>
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800&family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<style>
@page{size:A4 portrait;margin:0}
*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0;padding:0}
body{font-family:'Inter','Cairo',sans-serif;background:#fff;color:#1e293b;font-size:9px;line-height:1.4}
.inv{width:210mm;min-height:297mm;margin:auto;border:2px solid #991b1b;display:flex;flex-direction:column;position:relative;overflow:hidden}
.inv::before{content:'';position:absolute;top:0;left:0;right:0;height:6px;background:linear-gradient(90deg,#991b1b,#dc2626,#ef4444,#991b1b)}
.inv::after{content:'';position:absolute;bottom:0;left:0;right:0;height:6px;background:linear-gradient(90deg,#991b1b,#ef4444,#dc2626,#991b1b)}
.hdr{display:flex;justify-content:space-between;padding:12px 18px;background:linear-gradient(135deg,#991b1b,#dc2626);color:#fff;gap:14px;margin-top:6px;align-items:center}
.cblk{display:flex;gap:12px;flex:1;align-items:center}
.logo{width:55px;height:55px;object-fit:cover;border-radius:8px;background:rgba(255,255,255,0.1);padding:3px;border:2px solid rgba(251,191,36,0.4)}
.ct h2{font-size:15px;font-weight:800;color:#fbbf24;margin:0;font-family:'Cairo'}
.ct h1{font-size:9px;font-weight:600;color:rgba(255,255,255,0.85);text-transform:uppercase;letter-spacing:1.5px;margin:3px 0 0}
...
.comp-det .ci{display:flex;flex-direction:column;gap:1px}
.comp-det .cl{font-size:7px;color:#991b1b;font-weight:600;text-transform:uppercase;letter-spacing:0.3px}
.comp-det .cv{font-size:9px;color:#7f1d1d;font-weight:600}
.body{padding:12px 18px;flex:1;display:flex;flex-direction:column;gap:10px}
.sec-title{font-size:9px;font-weight:700;text-transform:uppercase;color:#991b1b;margin-bottom:5px;border-bottom:2px solid #991b1b;padding-bottom:4px;letter-spacing:0.5px}
.ib{padding:10px;background:#fef2f2;border-radius:6px;border:1px solid #fecaca;border-${textAlign === 'right' ? 'right' : 'left'}:4px solid #dc2626}
.ir{display:flex;justify-content:space-between;font-size:9px;padding:3px 0;border-bottom:1px solid #fee2e2}
.ir:last-child{border:none}.ir .l{color:#991b1b;font-weight:500}.ir .v{color:#7f1d1d;font-weight:600;text-align:${textAlignR}}
.ticket-compare{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:8px 0}
.ticket-box{padding:12px;border-radius:8px;border:2px solid}
.ticket-box.old{background:#fef2f2;border-color:#fca5a5}
.ticket-box.new-box{background:#f0fdf4;border-color:#86efac}
.ticket-box h4{font-size:9px;margin:0 0 8px;font-weight:700;text-transform:uppercase}
.ticket-box.old h4{color:#dc2626}
.ticket-box.new-box h4{color:#059669}
.ticket-box .val{font-size:14px;font-weight:800;padding:6px 0}
.ticket-box.old .val{color:#991b1b}
.ticket-box.new-box .val{color:#065f46}
.loss-banner{background:linear-gradient(135deg,#fef2f2,#fee2e2);border:2px solid #fca5a5;border-radius:8px;padding:12px;display:flex;justify-content:space-between;align-items:center}
.loss-banner .label{font-size:10px;color:#991b1b;font-weight:700;text-transform:uppercase}
.loss-banner .amount{font-size:22px;font-weight:800;color:#dc2626}
.deduction-badge{display:inline-block;padding:6px 16px;border-radius:20px;font-size:10px;font-weight:700;margin-top:8px;${paidByEmp ? 'background:#dcfce7;color:#065f46;border:2px solid #86efac' : 'background:#f8fafc;color:#64748b;border:2px solid #e2e8f0'}}
.notes-box{padding:10px;background:#fffbeb;border-radius:6px;border:1px solid #fde68a}
.notes-box h4{font-size:8px;color:#92400e;margin:0 0 4px;font-weight:700;text-transform:uppercase}
.notes-box p{font-size:8px;color:#78350f;margin:0;line-height:1.6}
.ft{padding:10px 18px;background:linear-gradient(135deg,#fef2f2,#fee2e2);display:flex;justify-content:space-between;align-items:center;border-top:2px solid #fecaca;gap:10px;margin-top:auto;min-height:60px}
.code-box{display:flex;align-items:center;gap:10px}
.barcode-img{height:35px;width:auto;min-width:150px;border:2px solid #fecaca;padding:2px 5px;background:#fff;border-radius:4px}
.qr-img{height:45px;width:45px;border:2px solid #fecaca;padding:2px;background:#fff;border-radius:5px}
...
.ai-label{font-size:7px;color:#dc2626;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:3px}
.ft-info{width:90px;text-align:center}
.ft-info p{font-size:7px;color:#7f1d1d;margin:2px 0}
.sig-area{display:flex;justify-content:space-between;padding:20px 18px 10px;margin-top:auto}
.sig-box{text-align:center;width:40%}
.sig-line{border-top:1px solid #991b1b;margin-top:40px;padding-top:5px;font-size:8px;color:#991b1b;font-weight:600}
@media print{body{background:#fff;padding:0;margin:0}.inv{border:none}}
</style></head><body>
<div class="inv">
<div class="hdr">
  ...
  <div class="im">
    <h3>⚠️ STAFF MISTAKE<span>خطأ موظف</span></h3>
    <div class="mr"><span class="l">Report No / رقم</span><span class="v">${mno}</span></div>
    <div class="mr"><span class="l">Date / التاريخ</span><span class="v">${mistake.date || ''}</span></div>
    <div class="mr"><span class="l">Status / الحالة</span><span class="v">${paidByEmp ? 'DEDUCTED' : 'PENDING'}</span></div>
  </div>
</div>
<div class="comp-det">
  <div class="ci"><span class="cl">VAT No / رقم ضريبي</span><span class="cv">${st.vat_no || 'N/A'}</span></div>
...
<div class="body">
  <div class="ib">
    <div class="sec-title">EMPLOYEE DETAILS / بيانات الموظف</div>
    <div class="ir"><span class="l">Name / الاسم</span><span class="v">${mistake.employees?.name || 'N/A'}</span></div>
    <div class="ir"><span class="l">Position / المسمى</span><span class="v">${mistake.employees?.position || mistake.employees?.role || 'N/A'}</span></div>
    <div class="ir"><span class="l">Department / القسم</span><span class="v">${mistake.employees?.department || 'N/A'}</span></div>
  </div>
  <div class="ticket-compare">
    <div class="ticket-box old">
      <h4>❌ OLD TICKET / التذكرة القديمة</h4>
      <div class="val">${mistake.old_ticket_no || 'N/A'}</div>
    </div>
    <div class="ticket-box new-box">
      <h4>✅ NEW TICKET / التذكرة الجديدة</h4>
      <div class="val">${mistake.new_ticket_no || 'N/A'}</div>
    </div>
  </div>
  <div class="loss-banner">
    <div class="label">💰 LOSS AMOUNT / مبلغ الخسارة</div>
    <div class="amount">${lossAmt.toFixed(2)} SAR</div>
  </div>
  <div style="text-align:center">
    <div class="deduction-badge">${paidByEmp ? '✅ DEDUCTED FROM SALARY / خصم من الراتب' : '⏳ NOT DEDUCTED / لم يُخصم'}</div>
  </div>
  ${mistake.reason ? `<div class="notes-box"><h4>📝 Reason / السبب</h4><p>${mistake.reason}</p></div>` : ''}
</div>
<div class="sig-area">
  <div class="sig-box"><div class="sig-line">Employee Signature / توقيع الموظف</div></div>
  <div class="sig-box"><div class="sig-line">Manager Signature / توقيع المدير</div></div>
...
<div class="ft">
  <div class="code-box">
    <div style="text-align:center"><img src="${barcode}" alt="Barcode" class="barcode-img" crossorigin="anonymous"/><div class="code-label">MISTAKE BARCODE<span>باركود الخطأ</span></div></div>
    <div style="text-align:center"><img src="${qr}" alt="QR Code" class="qr-img" crossorigin="anonymous"/><div class="code-label">DOWNLOAD QR<span>تحميل QR</span></div></div>
  </div>
  <div class="ft-divider"></div>
  <div class="ai-msg"><div class="ai-label">🤖 AI MESSAGE</div><p>${aiMsg}<span>${lang === 'ar' ? '⚠️ تقرير خطأ مسجل' : 'Mistake report logged'}</span></p></div>
  <div class="ft-divider"></div>
  <div class="ft-info"><p style="font-weight:700;color:#7f1d1d">${st.company_name_en || ''}</p><p>${st.phone || ''}</p><p style="font-family:'Cairo'">${st.company_name_ar || ''}</p></div>
</div></div></body></html>`;
};
