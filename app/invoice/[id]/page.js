...
export default function PublicInvoicePage({ params }) {
  const { id } = params; // FIX: Changed invNo to id
  ...
  return (
    <div style={{ background: '#f1f5f9', minHeight: '100vh', padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: "'Poppins', sans-serif" }}>
      <button onClick={handleDownload} disabled={loading} style={{ padding: '15px 30px', background: '#1E3A8A', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', marginBottom: '20px', fontSize: '16px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
      ...
      <div id="printable-invoice" style={{ width: '794px', background: 'white', padding: '40px', boxShadow: '0 0 10px rgba(0,0,0,0.1)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', borderBottom:'3px solid #1E3A8A', paddingBottom:'20px', marginBottom:'30px' }}>
          <div>
            ...
            <h1 style={{ margin:0, color:'#1E3A8A' }}>{settings.company_name_en || 'SUEUD AL TAAYIRA'}</h1>
            <h2 style={{ margin:'5px 0', color:'#555' }}>{settings.company_name_ar || 'صعود الطائرة للسفر السياحة'}</h2>
          ...
          <div style={{ textAlign:'right' }}>
            <h2 style={{ color: '#1E3A8A' }}>TAX INVOICE</h2>
