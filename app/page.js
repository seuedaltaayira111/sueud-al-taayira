...
export default function Home() {
  ...
  const fetchAll = async () => {
    ...
    const cbk = await supabase.from('cashbook').select('*').order('trans_date', { ascending: false }).limit(100);
      ...
      html.innerHTML = `
        <div style="display:flex;justify-content:space-between;border-bottom:4px solid #D4AF37;padding-bottom:20px;margin-bottom:20px;">
          <div style="max-width:350px;">
            ...
            <p style="font-size:12px;margin-top:10px;line-height:1.5;">VAT: ${s.vat_no || ''}<br/>CR: ${s.cr_no || ''}<br/>Phone: ${s.phone || ''}</p>
          </div>
        ...
        <div style="margin-bottom:20px;border:1px solid #eee;padding:15px;background:#f9f9f9;border-radius:8px;">
          <b>Customer:</b> ${inv.customers?.name || ''} | <b>Phone:</b> ${inv.customers?.phone || ''}<br/>
          <b>Sector/Service:</b> ${inv.sector || inv.service_type}
        </div>
        <table style="width:100%;border-collapse:collapse;text-align:center;font-size:12px;margin-bottom:20px;">
          <thead><tr style="background:#0F3D2E;color:#fff;">
            <th style="padding:8px;border:1px solid #ddd;">Qty</th><th style="padding:8px;border:1px solid #ddd;">Amount</th>
          </tr></thead>
          <tbody><tr>
            <td style="padding:8px;border:1px solid #ddd;">${inv.qty || 1}</td><td style="padding:8px;border:1px solid #ddd;">${inv.total_sell.toFixed(2)} SAR</td>
          </tr></tbody>
        </table>
        <div style="display:flex;justify-content:space-between;margin-top:30px;">
          ...
          <div style="text-align:right;width:300px;font-size:14px;">
            <p style="display:flex;justify-content:space-between;border-bottom:1px solid #eee;padding:5px 0;"><span>Total:</span><b>${inv.total.toFixed(2)} SAR</b></p>
            <p style="display:flex;justify-content:space-between;padding:5px 0;color:#27ae60;"><span>Paid:</span><b>${inv.paid_amount.toFixed(2)} SAR</b></p>
            <p style="display:flex;justify-content:space-between;padding:5px 0;color:#e74c3c;font-weight:bold;"><span>Due:</span><b>${inv.due_amount.toFixed(2)} SAR</b></p>
          </div>
        </div>
      `;
    ...
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'Arial', backgroundColor: '#F5F7F2', direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
      ...
      <main style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ padding: '30px' }}>
          ...
          {['refunds', 'portals', 'bank', 'invest', 'hr', 'users', 'reports', 'audit', 'settings'].includes(page) && (
             <div style={{ background: 'white', padding: '30px', borderRadius: '8px', borderTop: '4px solid #D4AF37' }}>
               <h3>{menu.find(m=>m.id===page)?.label} Module</h3>
               <p>This section is intact in your original code. I focused on upgrading Dashboard, Create Invoice, List, and Customers to show new features.</p>
             </div>
          )}
