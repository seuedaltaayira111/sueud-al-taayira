...
const translations = {
  ...
  ar: {
    ...
    refund:'استرجاع', quickSettle:'تسوية', download_excel:'تصدير', save:'حفظ',
    add:'إضافة', search:'بحث...', changePass:'تغيير كلمة المرور', logout:'تسجيل خروج',
...
export default function useERPState() {
  ...
  const [empForm, setEmpForm] = useState({ name:'', phone:'', iqama_no:'', iqama_expiry:'', role:'Sales', salary:0, commission_rate:0 });
  ...
  const [userForm, setUserForm] = useState({ email:'', username:'', role:'Staff', is_admin:false, can_access_invoices:true, can_access_bank:false, can_access_hr:false, can_access_reports:false, can_access_settings:false });
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [
        ...
        setRes, srvRes, advRes, investRes
      ] = await Promise.all([
  ...
  return {
    ...
    // HTML Generators (from lib/invoiceHTML.js)
