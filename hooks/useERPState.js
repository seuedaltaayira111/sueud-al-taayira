...
const getAirlineCheckInURL = (airline, pnr) => {
  ...
  const urls = {
    ...
    'indigo': `https://www.goindigo.in/manage-booking?pnr=${pnr}`, 'virgin': `https://www.virginatlantic.com/check-in?pnr=${pnr}`,
  };
...
const getAIMessage = (inv, lang) => {
  const msgs = [
    ...
    { en: "🌟 Your trust means the world to us.", ar: "🌟 ثقتكم تعني لنا كل شيء." },
...
.im{min-width:160px;background:rgba(255,255,255,0.06);padding:10px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);text-align:${al}}
...
.cd{display:grid;grid-template-columns:repeat(4,1fr);gap:4px;padding:6px 16px;background:#f0f4ff;border-bottom:1px solid #e2e8f0}
.cd .ci{display:flex;flex-direction:column;gap:0}.cd .cl{font-size:7px;color:#64748b;font-weight:600;text-transform:uppercase}.cd .cv{font-size:8px;color:#0f172a;font-weight:600}
.body{padding:8px 16px;flex:1;display:flex;flex-direction:column;gap:6px}
.stl{font-size:8px;font-weight:700;text-transform:uppercase;color:#0c1d3a;margin-bottom:4px;border-bottom:1.5px solid #0c1d3a;padding-bottom:3px}
...
.ir:last-child{border:none}.ir .l{color:#64748b}.ir .v{color:#0f172a;font-weight:600;text-align:${ar};max-width:65%;word-break:break-word}
...
thead th{padding:5px 6px;background:#0c1d3a;color:#fbbf24;font-size:7px;text-transform:uppercase;text-align:${al};border-bottom:1.5px solid #fbbf24}
thead th.r{text-align:${ar}}thead th.c{text-align:center}
tbody td{padding:4px 6px;border-bottom:1px solid #f1f5f9;font-size:8px}
tbody td.r{text-align:${ar};font-weight:600}tbody td.c{text-align:center}
...
.bi{height:28px;width:auto;min-width:160px;border:1px solid #e2e8f0;padding:1px 4px;background:#fff;border-radius:2px}
.qi{height:36px;width:36px;border:1px solid #e2e8f0;padding:1px;background:#fff;border-radius:3px}
.cl2{font-size:6px;font-weight:700;text-transform:uppercase;text-align:center;color:#475569}
.am{text-align:center;flex:1;padding:4px 10px;background:linear-gradient(135deg,#eff6ff,#dbeafe);border-radius:6px;border:1px solid #93c5fd}
.am p{font-size:8px;color:#1e3a8a;margin:0;line-height:1.4;font-weight:500}
.al{font-size:6px;color:#3b82f6;font-weight:700;text-transform:uppercase;margin-bottom:2px}
.fi{width:70px;text-align:center}.fi p{font-size:6px;color:#94a3b8;margin:1px 0}
.fd{width:1px;background:#cbd5e1;align-self:stretch;min-height:40px}
@media print{body{background:#fff;padding:0;margin:0}.inv{border:none}}
</style></head><body>
<div class="inv">
<div class="hdr"><div class="cblk">
...
<div class="ct"><h2>${st.company_name_ar||'صعود الطائرة للسفر والسياحة'}</h2><h1>${st.company_name_en||'SUEUD AL TAAYIRA TRAVEL & TOURISM'}</h1><p>${st.address_ar||'Address'} ${st.phone?'| '+st.phone:''}</p></div>
</div><div class="im"><h3>TAX INVOICE<span>فاتورة ضريبية</span></h3>
...
<div class="sb">${st2==='Unpaid'?'UNPAID / غير مدفوعة':'PAID / مدفوعة'}</div></div></div>
<div class="cd">
...
<div class="body"><div class="dg">
<div class="ib"><div class="stl">BILL TO / فاتورة إلى</div>
...
<div class="stl">FLIGHT DETAILS / تفاصيل الرحلة</div>
...
<div class="ir"><span class="l">Service / الخدمة</span><span class="v">${inv.service_type||'N/A'}</span></div>
 ...
 ${isRe?`<div class="rb"><div class="rt"><span>⚠️ PREVIOUS BOOKING</span></div><div class="rg">
...
<div class="rf"><div class="l">Original Fare</div><div class="v">${parseFloat(inv.old_sell_price||0).toFixed(2)} SAR</div></div>
</div></div>`:''}
...
<tr><td>${inv.sector||inv.service_type||'Service'}</td><td class="c">${inv.qty||1}</td><td class="r">${up.toFixed(2)}</td><td class="r">${ts.toFixed(2)}</td></tr>
 ${disc>0?`<tr style="background:#f0fdf4"><td colspan="3" style="text-align:${ar};color:#059669">Discount / خصم</td><td class="r" style="color:#059669">- ${disc.toFixed(2)}</td></tr>`:''}
</tbody></table>
<div class="bs"><div class="pb"><div class="stl">PAYMENT BREAKDOWN</div>
<div class="pr"><span>Subtotal</span><span style="font-weight:600">${sub.toFixed(2)} SAR</span></div>
 ${disc>0?`<div class="pr" style="color:#059669"><span>Discount</span><span>- ${disc.toFixed(2)} SAR</span></div>`:''}
...
<div class="pr" style="font-weight:700;font-size:10px"><span>Due</span><span style="color:${due>0?'#ef4444':'#059669'}">${due.toFixed(2)} SAR</span></div>
</div><div class="tb">
...
<div class="terms"><p>1. Bookings subject to airline/hotel terms. 2. Cancellation policies vary. 3. Computer-generated - valid without signature. 4. Prices in SAR incl. VAT. 5. Electronic invoice under Fatoorah.</p></div>
</div><div class="ft"><div class="cb">
<img src="${barcode}" alt="Barcode" class="bi" crossorigin="anonymous"/><div class="cl2">CHECK-IN</div>
<img src="${qr}" alt="QR" class="qi" crossorigin="anonymous"/><div class="cl2">INVOICE</div>
</div><div class="fd"></div><div class="am"><div class="al">🤖 AI</div><p>${aiMsg}</p></div>
<div class="fd"></div><div class="fi"><p style="font-weight:700;color:#0f172a;font-size:7px">${st.company_name_en||'SUEUD AL TAAYIRA'}</p><p>${st.phone||''}</p><p style="font-family:'Cairo';font-size:7px">${st.company_name_ar||''}</p></div>
</div></div></body></html>`;
};
...
.im{min-width:160px;text-align:${al}}
...
.cd{display:grid;grid-template-columns:repeat(4,1fr);gap:4px;padding:6px 16px;background:#fff5f5;border-bottom:1px solid #fecaca}
.cd .ci{display:flex;flex-direction:column;gap:0}.cd .cl{font-size:7px;color:#991b1b;font-weight:600;text-transform:uppercase}.cd .cv{font-size:8px;color:#7f1d1d;font-weight:600}
.body{padding:10px 16px;flex:1;display:flex;flex-direction:column;gap:8px}
.stl{font-size:8px;font-weight:700;text-transform:uppercase;color:#7f1d1d;margin-bottom:4px;border-bottom:1.5px solid #7f1d1d;padding-bottom:3px}
...
.row:last-child{border:none}.row .l{color:#991b1b;font-weight:500}.row .v{color:#7f1d1d;font-weight:600;text-align:${ar};max-width:65%}
...
.qi{height:36px;width:36px;border:1px solid #fecaca;padding:1px;background:#fff;border-radius:3px}
.cl2{font-size:6px;font-weight:700;text-transform:uppercase;text-align:center;color:#991b1b}
.fd{width:1px;background:#fecaca;align-self:stretch;min-height:40px}
.am{text-align:center;flex:1;padding:4px 10px;background:linear-gradient(135deg,#fef2f2,#fee2e2);border-radius:6px;border:1px solid #fca5a5}
.am p{font-size:8px;color:#7f1d1d;margin:0;line-height:1.4;font-weight:500}
.al{font-size:6px;color:#dc2626;font-weight:700;text-transform:uppercase;margin-bottom:2px}
.fi{width:70px;text-align:center}.fi p{font-size:6px;color:#94a3b8;margin:1px 0}
@media print{body{background:#fff;padding:0;margin:0}.inv{border:none}}
</style></head><body>
<div class="inv"><div class="hdr"><div class="cblk">
 ${st.logo_url?`<img src="${st.logo_url}" crossorigin="anonymous" class="logo"/>`:'<div class="logo" style="display:flex;align-items:center;justify-content:center;font-size:20px">✈️</div>'}
...
</div><div class="im"><h3>REFUND<span>استرجاع</span></h3><div class="ino">No: <span>${no}</span></div><div class="ino">Date: <span>${inv.refund_date||inv.invoice_date||''}</span></div><div class="sb">PROCESSED / تم</div></div></div>
...
<div class="body"><div class="ib"><div class="stl">BOOKING DETAILS</div>
<div class="row"><span class="l">Customer</span><span class="v">${cn}</span></div>
<div class="row"><span class="l">Phone</span><span class="v">${cp}</span></div>
<div class="row"><span class="l">Passengers</span><span class="v" style="font-size:7px">${pax}</span></div>
<div class="row"><span class="l">Airline</span><span class="v">${inv.airline||inv.old_airline||'N/A'}</span></div>
<div class="row"><span class="l">Date</span><span class="v">${inv.invoice_date||'N/A'}</span></div>
<div class="row"><span class="l">PNR</span><span class="v">${inv.pnr||inv.old_pnr||'N/A'}</span></div>
<div class="row"><span class="l">Reason</span><span class="v">${inv.refund_reason||'N/A'}</span></div>
</div><div class="cb"><div class="ct2">REFUND CALCULATION</div>
<div class="cr"><span>Original Fare</span><span style="font-weight:600">${of2.toFixed(2)} SAR</span></div>
<div class="cr deduct"><span>Less: Airline Fees</span><span style="font-weight:600">- ${(of2-compRef).toFixed(2)} SAR</span></div>
<div class="cr"><span>Refund to Portal</span><span style="font-weight:600;color:#2563eb">${compRef.toFixed(2)} SAR</span></div>
<div class="cr total"><span>Refund to Customer</span><span>${cRef.toFixed(2)} SAR</span></div>
</div><div class="pi"><span style="font-weight:600;color:#334155">Refund Method</span><span style="font-weight:600;color:#2563eb">${inv.payment_method||'Cash'}</span></div>
</div><div class="ft"><div style="display:flex;align-items:center;gap:6px"><img src="${qr}" alt="QR" class="qi" crossorigin="anonymous"/><div class="cl2">DOWNLOAD</div></div>
<div class="fd"></div><div class="am"><div class="al">🤖 AI</div><p>${aiMsg}</p></div>
<div class="fd"></div><div class="fi"><p style="font-weight:700;color:#7f1d1d">${st.company_name_en||''}</p><p>${st.phone||''}</p><p style="font-family:'Cairo'">${st.company_name_ar||''}</p></div>
</div></div></body></html>`;
};
...
.cd{display:grid;grid-template-columns:repeat(4,1fr);gap:4px;padding:6px 16px;background:#fff7ed;border-bottom:1px solid #fed7aa}
.cd .ci{display:flex;flex-direction:column;gap:0}.cd .cl{font-size:7px;color:#9a3412;font-weight:600;text-transform:uppercase}.cd .cv{font-size:8px;color:#7c2d12;font-weight:600}
.body{padding:10px 16px;flex:1;display:flex;flex-direction:column;gap:8px}
.stl{font-size:8px;font-weight:700;text-transform:uppercase;color:#7c2d12;margin-bottom:4px;border-bottom:1.5px solid #7c2d12;padding-bottom:3px}
...
.am{text-align:center;flex:1;padding:4px 10px;background:linear-gradient(135deg,#fff7ed,#fed7aa);border-radius:6px;border:1px solid #fdba74}
.am p{font-size:8px;color:#7c2d12;margin:0;line-height:1.4;font-weight:500}
.al{font-size:6px;color:#9a3412;font-weight:700;text-transform:uppercase;margin-bottom:2px}
.fi{width:70px;text-align:center}.fi p{font-size:6px;color:#94a3b8;margin:1px 0}
@media print{body{background:#fff;padding:0;margin:0}.inv{border:none}}
</style></head><body>
<div class="inv"><div class="hdr"><div class="ci"><h2>${st.company_name_ar||'صعود الطائرة'}</h2><h1>${st.company_name_en||'SUEUD AL TAAYIRA'}</h1></div>
<div class="im"><h3>EXPENSE VOUCHER</h3><p>No: <span>${eno}</span></p><p>Date: <span>${exp.expense_date||''}</span></p></div></div>
<div class="cd"><div class="ci"><span class="cl">VAT No</span><span class="cv">${st.vat_no||'N/A'}</span></div><div class="ci"><span class="cl">CR No</span><span class="cv">${st.cr_no||'N/A'}</span></div><div class="ci"><span class="cl">License</span><span class="cv">${st.license_no||'N/A'}</span></div><div class="ci"><span class="cl">Tourism Lic.</span><span class="cv">${st.tourism_license_no||'N/A'}</span></div></div>
<div class="body"><div class="ib"><div class="stl">DETAILS</div>
<div class="row"><span class="l">Type</span><span class="v">${exp.expense_type||'N/A'}</span></div>
<div class="row"><span class="l">Payment</span><span class="v">${exp.payment_mode||'Cash'}</span></div>
<div class="row"><span class="l">Description</span><span class="v">${exp.description||'N/A'}</span></div>
</div><table><thead><tr><th>Item</th><th class="r">Qty</th><th class="r">Price</th><th class="r">Total</th></tr></thead>
<tbody>${items.map(function(it){return '<tr><td>'+(it.name||'Item')+'</td><td class="r">'+(it.qty||1)+'</td><td class="r">'+parseFloat(it.price||0).toFixed(2)+'</td><td class="r">'+((parseFloat(it.qty)||0)*(parseFloat(it.price)||0)).toFixed(2)+'</td></tr>';}).join('')}</tbody></table>
<div class="totals"><h3>TOTAL</h3><p class="amt">${(exp.amount||0).toFixed(2)} SAR</p></div></div>
<div class="ft"><div class="fi"><p style="font-weight:700;color:#7c2d12">${st.company_name_en||''}</p><p>${st.phone||''}</p><p style="font-family:'Cairo'">${st.company_name_ar||''}</p></div>
<div class="am"><div class="al">🤖 AI</div><p>${aiMsg}</p></div><div style="width:70px"></div></div>
</div></body></html>`;
};
...
.cd{display:grid;grid-template-columns:repeat(4,1fr);gap:4px;padding:6px 16px;background:#EFF6FF;border-bottom:1px solid #BFDBFE}
.cd .ci{display:flex;flex-direction:column;gap:0}.cd .cl{font-size:7px;color:#1E3A8A;font-weight:600;text-transform:uppercase}.cd .cv{font-size:8px;color:#0F172A;font-weight:600}
.body{padding:10px 16px;flex:1;display:flex;flex-direction:column;gap:8px}
.stl{font-size:8px;font-weight:700;text-transform:uppercase;margin-bottom:4px;border-bottom:1.5px solid #0F172A;padding-bottom:3px}
...
.earn .stl{color:#059669;border-color:#059669}.ded .stl{color:#EF4444;border-color:#EF4444}
...
<div class="slip"><div class="hdr"><h1>${st.company_name_ar||'صعود الطائرة'}</h1>
...
<div class="cd"><div class="ci"><span class="cl">VAT No</span><span class="cv">${st.vat_no||'N/A'}</span></div><div class="ci"><span class="cl">CR No</span><span class="cv">${st.cr_no||'N/A'}</span></div><div class="ci"><span class="cl">License</span><span class="cv">${st.license_no||'N/A'}</span></div><div class="ci"><span class="cl">Tourism Lic.</span><span class="cv">${st.tourism_license_no||'N/A'}</span></div></div>
<div class="body"><div class="dg">
<div class="ib earn"><div class="stl">EARNINGS</div>
<div class="row"><span class="l">Base Salary</span><span class="v">${(pay.base_salary||0).toFixed(2)}</span></div>
<div class="row"><span class="l">Commission</span><span class="v">${(pay.commission_amount||0).toFixed(2)}</span></div>
<div class="row"><span class="l">Overtime</span><span class="v">${(pay.overtime||0).toFixed(2)}</span></div>
<div class="row"><span class="l">Gift/Bonus</span><span class="v">${(pay.gift||0).toFixed(2)}</span></div>
<div class="row" style="font-weight:700;border-top:1px solid #059669;margin-top:3px;padding-top:3px"><span class="l">GROSS</span><span class="v" style="color:#059669">${gross.toFixed(2)}</span></div>
</div><div class="ib ded"><div class="stl">DEDUCTIONS</div>
<div class="row"><span class="l">Advance</span><span class="v">${(pay.advance_deduction||0).toFixed(2)}</span></div>
<div class="row"><span class="l">Mistakes</span><span class="v">${(pay.mistakes_deduction||0).toFixed(2)}</span></div>
<div class="row"><span class="l">Other</span><span class="v">${(pay.other_deduction||0).toFixed(2)}</span></div>
<div class="row" style="font-weight:700;border-top:1px solid #EF4444;margin-top:3px;padding-top:3px"><span class="l">TOTAL DED.</span><span class="v" style="color:#EF4444">${tded.toFixed(2)}</span></div>
</div><div class="ib" style="border-left:3px solid #2563EB">
<div class="stl" style="color:#2563EB;border-color:#2563EB">SUMMARY</div>
<div class="row"><span class="l">Mode</span><span class="v">${pay.payment_mode||'Cash'}</span></div>
<div class="row"><span class="l">Working Days</span><span class="v">${pay.working_days||'N/A'}</span></div>
<div class="row"><span class="l">Leaves</span><span class="v">${pay.leaves_taken||0}</span></div>
<div class="row" style="font-weight:700;border-top:1px solid #2563EB;margin-top:3px;padding-top:3px"><span class="l">NET PAY</span><span class="v" style="color:#2563EB;font-size:12px">${net.toFixed(2)}</span></div>
</div></div>
<div class="totals"><h3>NET SALARY</h3><p class="amt">${net.toFixed(2)} SAR</p></div></div>
<div class="ft"><div class="fi"><p style="font-weight:700;color:#0F172A">${st.company_name_en||''}</p><p>${st.phone||''}</p><p style="font-family:'Cairo'">${st.company_name_ar||''}</p></div>
<div class="am"><div class="al">🤖 AI</div><p>${aiMsg}</p></div><div style="width:70px"></div></div>
</div></body></html>`;
};
...
<style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Inter','Cairo',sans-serif;background:#fff;color:#1e293b;font-size:11px;padding:40px}h1{color:#0c1d3a;font-size:24px;margin-bottom:20px;text-align:center}h2{color:#1a365d;font-size:16px;margin:20px 0 10px;border-bottom:2px solid #fbbf24;padding-bottom:5px}.ig{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:20px 0}.ii{background:#f8fafc;padding:10px;border-radius:6px;border-left:3px solid #1a365d}.ii .l{font-size:9px;color:#64748b;text-transform:uppercase}.ii .v{font-size:13px;font-weight:600;color:#0f172a;margin-top:2px}.tm{background:#f8fafc;padding:15px;border-radius:8px;margin:20px 0;line-height:1.8}.sg{display:flex;justify-content:space-between;margin-top:60px}.sb{width:200px;text-align:center}.sl{border-bottom:1px solid #1e293b;margin-bottom:5px;height:60px}</style></head><body>
...
<div class="tm">${terms||'Standard terms apply. All bookings subject to airline/hotel terms and availability. Prices in SAR including VAT.'}</div>
...
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700&family=Inter:wght@400;600;700&display=swap" rel="stylesheet">
<style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Inter','Cairo',sans-serif;background:#fff;color:#1e293b;font-size:11px;padding:40px}h1{color:#7f1d1d;font-size:20px;margin-bottom:20px;text-align:center}.info{background:#fff5f5;padding:15px;border-radius:8px;border-left:3px solid #dc2626;margin:20px 0}.row{display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px dashed #fecaca}.row:last-child{border:none}.total{background:#7f1d1d;color:#fff;padding:15px;border-radius:8px;display:flex;justify-content:space-between;align-items:center;margin-top:20px;font-size:16px;font-weight:700}.total span:last-child{color:#fbbf24}</style></head><body>
...
/* ═══════════════════════════════════════════════════════════════
   MAIN HOOK — EXACT MATCH WITH useERPActions EXPECTATIONS
   ═══════════════════════════════════════════════════════════════ */
export default function useERPState() {
  ...
  /* ── DATA — exact keys that useERPActions uses ── */
  const [data, setData] = useState({
    invoices: [], customers: [], corporates: [], creditors: [],
  ...
  /* ── FORMS — exact field names that useERPActions uses ── */
  ...
  const safeLoad = useCallback(async (table, key, opts) => {
    ...
    try {
      ...
      if (opts.single) { var r = await q.single(); if (r.data) setData(function(p) { var n = Object.assign({}, p); n[key] = r.data; return n; }); }
      else { var r2 = await q; if (r2.data) setData(function(p) { var n = Object.assign({}, p); n[key] = r2.data; return n; }); }
    } catch (e) { console.warn('[ERP] Skip ' + table + ':', e.message); }
  }, []);

  /* ── FETCH ALL — correct table names and selects ── */
  ...
  const logAction = useCallback(async (details) => {
    try { await supabase.from('audit_logs').insert([{ action:'user_action', details:details, user_id:user?.id, tenant_id:userProfile?.tenant_id }]); } catch(e) {}
  }, [user, userProfile]);
  ...
  useEffect(function() {
    const init = async () => {
      try {
        ...
        var profile = { id:session.user.id, email:session.user.email, name:session.user.user_metadata?.name||session.user.email?.split('@')[0]||'User', role:'Admin', is_admin:true, can_access_hr:true, can_access_bank:true, can_access_invoices:true, can_access_reports:true, can_access_settings:true, tenant_id:session.user.user_metadata?.tenant_id||'default' };
        try { var profResult = await supabase.from('app_users').select('*').eq('id', session.user.id).single(); if (profResult.data) profile = Object.assign({}, profile, profResult.data); } catch(e) { console.warn('[ERP] app_users not found, using default profile'); }
        setUserProfile(profile);
        await fetchAll();
      } catch(e) { console.error('[ERP] Init:', e); setInitError(e.message||'Init failed'); }
    };
    init();
  }, [router, fetchAll]);
