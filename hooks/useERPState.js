...
import { getInvoiceHTML, getRefundHTML, getExpenseHTML, getSalarySlipHTML, getMistakeHTML, getContractHTML } from '@/lib/htmlTemplates';
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (!profile) {
          setInitError('No profile found for your account. Contact admin to setup your access.');
          setLoading(false);
          return;
        }
        ...
        if (!profile.is_super_admin) {
          ...
          if (tenant && !tenant.is_paid && tenant.subscription_end_date && new Date(tenant.subscription_end_date) < new Date()) {
            setInitError('Subscription expired on ' + tenant.subscription_end_date + '. Please renew.');
            setLoading(false);
