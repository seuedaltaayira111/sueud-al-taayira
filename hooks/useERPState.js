  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [
        ...
        flightTicketsRes, ticketNotifRes
      ] = await Promise.all([
        ...
        supabase.from('flight_tickets').select('*').eq('tenant_id', tid).order('created_at', { ascending: false }),
        supabase.from('ticket_notifications').select('*').eq('tenant_id', tid).order('created_at', { ascending: false }).limit(50)
      ]);
      ...
      setData({
        ...
        flightTickets: flightTicketsRes.data || [],
        ticketNotifications: ticketNotifRes.data || []
      });
    } catch (err) {
