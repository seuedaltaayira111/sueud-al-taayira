...
export default function useERPActions(state) {
  ...
  const handleSendMessage = () => { 
    ...
    setChatInput(''); 
    setTimeout(() => { 
      setChatMessages(prev => [...prev, { sender: 'bot', text: "I can help with Invoices. (يمكنني المساعدة في الفواتير)" }]); 
    }, 600); 
  };
    ...
    try {
      ...
      if (inv.invoice_no.startsWith('REF-')) {
        ...
        if (inv.portal_id && inv.refund_company > 0) {
          ...
          if (portal) {
            const newPortalBal = (portal.current_balance || 0) - (inv.refund_company || 0);
        ...
        setData(prev => ({ 
          ...prev, 
          invoices: prev.invoices.map(i => i.invoice_no === originalInvId ? { ...i, status: 'Unpaid' } : i).filter(i => i.id !== inv.id), 
        ...
        showToast('Refund Invoice Deleted & Original Invoice Restored!');
        return;
      }
    ...
    try {
      ...
      if (editExpId) {
        ...
        const { data: nC, error: cbErr } = await supabase.from('cashbook').insert([{ 
          ...
          type: cbType, 
          description: `Expense: ${expForm.vendor_name} (${oldExp.invoice_no})`, 
          amount: totalAmount, 
          tenant_id: userProfile.tenant_id 
        }]).select().single();
        ...
        const { data: nC, error: cbErr } = await supabase.from('cashbook').insert([{ 
          trans_date: expForm.expense_date || today, 
          type: cbType, 
          description: `Expense: ${expForm.vendor_name} (${expNo})`, 
          amount: totalAmount, 
          tenant_id: userProfile.tenant_id 
        }]).select().single();
  ...
  const handleAddEditVend = async (e) => { 
    e.preventDefault(); 
    const pl = { ...vendorForm, tenant_id: userProfile.tenant_id }; 
    try { 
      if (editVendId) { 
  ...
  const handleAddEditPkg = async (e) => { 
    e.preventDefault(); 
    const pl = { name: pkgForm.name, price: parseFloat(pkgForm.price), description: pkgForm.desc, duration: pkgForm.duration, inclusions: pkgForm.inclusions, tenant_id: userProfile.tenant_id }; 
    try { 
      if (editPkgId) { 
  ...
  const handleAddEditSrv = async (e) => { 
    e.preventDefault(); 
    const pl = { ...srvForm, tenant_id: userProfile.tenant_id }; 
    try { 
      if (editSrvId) { 
  ...
  const handleAddInvestment = async (e) => { 
    ...
    try { 
      ...
      const { data: newInv, error: invErr } = await supabase.from('investments').insert([{ 
        investor_name: investForm.name, 
        amount: parseFloat(investForm.amount), 
        invest_date: investForm.date, 
      ...
      const { data: nC, error: cbErr } = await supabase.from('cashbook').insert([{ 
        trans_date: investForm.date, 
        type: cbType, 
        description: `Investment by ${investForm.name} (${finalReason})`, 
        amount: parseFloat(investForm.amount), 
        tenant_id: userProfile.tenant_id 
      }]).select().single(); 
      if (cbErr) throw cbErr; 
  ...
  const handleRecharge = async (e) => { 
    ...
    try { 
      ...
      const { data: newRec, error: recErr } = await supabase.from('recharges').insert([{ 
        portal_id: p.id, 
        amount, 
        recharge_date: e.target.date.value, 
      ...
      const { data: nC, error: cbErr } = await supabase.from('cashbook').insert([{ 
        ...
        type: cbType, 
        description: `Recharge for ${p.name}`, 
        amount, 
        tenant_id: userProfile.tenant_id 
      }]).select().single(); 
      if (cbErr) throw cbErr; 
  ...
  const handleEditUser = (u) => { 
    ...
    setUserForm({ 
      ...
      role: u.role, 
      is_admin: u.is_admin, 
      can_access_invoices: u.can_access_invoices, 
      can_access_bank: u.can_access_bank, 
      can_access_hr: u.can_access_hr, 
      can_access_reports: u.can_access_reports, 
  ...
  const handlePaySalary = async (e) => { 
    ...
    try { 
      ...
      showToast('Salary Paid! Auto Calculated.');
      e.target.reset(); 
    } catch (err) { 
      showToast('Error: ' + err.message); 
    } 
  ...
  const handleSettlePayment = async (e) => { 
    ...
    try { 
      ...
      const { data: nC, error: cbErr } = await supabase.from('cashbook').insert([{ 
        trans_date: settleForm.date, type: cbType, description: `Settlement for ${inv.invoice_no}`, amount: inv.due_amount, tenant_id: userProfile.tenant_id 
      }]).select().single(); 
      if (cbErr) throw cbErr; 
  ...
  const handleQuickSettle = async (inv) => {
    if (inv.invoice_no.startsWith('REF-')) return showToast('Refund invoices cannot be settled!');
    ...
    try {
      ...
      const cbType = 'Cash-In';
      const { data: nC, error: cbErr } = await supabase.from('cashbook').insert([{ 
        trans_date: today, type: cbType, description: `Quick Settlement for ${inv.invoice_no}`, amount: inv.due_amount, tenant_id: userProfile.tenant_id 
      }]).select().single();
      if (cbErr) throw cbErr;
  ...
  const handleRefund = async (e) => { 
    ...
    try { 
      ...
      const { data: upInv, error: invErr } = await supabase.from('invoices').update({ 
        status: 'refunded' 
      }).eq('id', inv.id).select(`*, customers(name), employees(name)`).single(); 
      if (invErr) throw invErr; 
      ...
      if (custRef > 0) { 
        if (refundForm.mode === 'Credit') { 
          ...
          const { data: nC, error: cbErr } = await supabase.from('cashbook').insert([{ 
            ...
            type: cbType, 
            description: `Refund to customer for ${inv.invoice_no}`, 
            amount: custRef, 
            tenant_id: userProfile.tenant_id 
          }]).select().single(); 
          if (cbErr) throw cbErr; 
  ...
  const openRefundModal = (inv) => { 
    const cust = data.customers.find(c => c.id === inv.customer_id);
    setRefundForm({ id: inv.id, date: today, compRefund: 0, custRefund: 0, mode: 'Cash', reason: '', portalId: inv.portal_id, creditBalance: cust?.store_credit || 0 }); 
    setModal({ type: 'refund', data: inv }); 
  };
