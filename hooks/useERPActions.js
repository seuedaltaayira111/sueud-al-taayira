...
export default function useERPActions(state) {
  ...
  const handleQuickSettle = async (inv) => {
    ...
    try {
      ...
      const { data: nC, error: cbErr } = await supabase.from('cashbook').insert([{ trans_date: today, type: cbType, description: `Quick Settlement for ${inv.invoice_no}`, amount: inv.due_amount }]).select().single();
      if (cbErr) throw cbErr;
