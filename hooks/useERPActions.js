...
export default function useERPActions(state) {
    ...
    try {
      ...
      if (editExpId) {
        ...
        const { data: nExp, error } = await supabase
          ...
          .single();
        if (error) throw error;
        // Automatically deduct this expense from Cash or Bank
        let newCbEntry = null;
        if (payload.payment_mode === 'Cash' || payload.payment_mode === 'Bank Transfer') {
          ...
          const { data: cb } = await supabase.from('cashbook').insert([{
            trans_date: payload.expense_date || today,
            type: cbType,
  ...
  // ============================================================
  // PORTAL RECHARGE – with Investor support
  // ============================================================
...
Thank you for choosing us!`;
  ...
  // ============================================================
  // EDIT INVOICE (navigate to create page)
  // ============================================================
  ...
  const handleAddMistake = async (e) => {
    ...
    try {
      const { data: newM, error } = await supabase
        .from('staff_mistakes')
        .insert([{
          employee_id: mistakeForm.employee_id,
          old_ticket_no: mistakeForm.old_ticket_no,
          new_ticket_no: mistakeForm.new_ticket_no,
          loss_amount: parseFloat(mistakeForm.loss_amount) || 0,
          paid_by_employee: mistakeForm.paid_by_employee || false,
          reason: mistakeForm.reason || '',
          date: today,
          tenant_id: userProfile.tenant_id
        }])
  ...
  const handleDeleteMistake = async (m) => {
    ...
    try {
      const { error } = await supabase.from('staff_mistakes').delete().eq('id', m.id);
      if (error) throw new Error(error.message);
  ...
  const handleDeletePayroll = async (pay) => {
    ...
    try {
      ...
      for (const cb of cbs) await supabase.from('cashbook').delete().eq('id', cb.id);
      const { error } = await supabase.from('payroll').delete().eq('id', pay.id);
      if (error) throw new Error(error.message);
      setData(prev => ({
        ...prev,
  ...
  const handleDeleteAdvance = async (adv) => {
    ...
    try {
      const { error } = await supabase.from('emp_advances').delete().eq('id', adv.id);
      if (error) throw new Error(error.message);
      setData(prev => ({
        ...prev,
