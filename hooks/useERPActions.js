...
export default function useERPActions(state) {
  ...
  const downloadPDF = async (htmlContent, filename = 'document.pdf') => {
    try {
      ...
      // Fixed width matching the A4 templates (210mm ≈ 794px @ 96dpi) so
      // html2canvas measures a consistent box regardless of viewport —
      // this was the root cause of documents rendering short/blank on the
      // printed page (canvas aspect ratio not matching true A4 210x297mm).
      const A4_PX_W = 794;
      const A4_PX_H = 1123; // 297mm @ 96dpi
      div.style.cssText = `position:absolute;left:-9999px;top:0;width:${A4_PX_W}px;`;
      const parser = new DOMParser();
      const doc = parser.parseFromString(htmlContent, 'text/html');
      // Pre-fetch BOTH the QR codes and the barcodes as base64 — leaving
      // any external image to load live during capture is what was
      // causing missing/blank barcodes (no CORS headers => html2canvas
      // silently drops them, or the whole render times out short).
      ...
      // Small delay lets web fonts (Cairo/Inter) finish applying before
      // the snapshot — otherwise text can render with the fallback font
      // metrics, subtly shifting layout height.
      await new Promise(r => setTimeout(r, 150));
      const canvas = await html2canvas(div, {
        scale: 2, useCORS: true, allowTaint: true, backgroundColor: '#ffffff',
        windowWidth: A4_PX_W, windowHeight: Math.max(A4_PX_H, div.scrollHeight)
      });
  ...
  // Investment TRANSACTION log used by the "Invest" page (Admin > invest).
  // This is distinct from handleAddEditInvestor above (investor profiles) —
  // this one just logs "who put in how much money, when, and why", and
  // records the matching cashbook movement.
  const handleAddInvestment = async (e) => {
    ...
    try {
      ...
      const { data: newInv, error } = await supabase.from('investments').insert([{
        investor_name: investForm.name, amount, invest_date: investForm.date || today,
        mode: investForm.mode || 'Cash', reason: finalReason, description: investForm.desc || '',
        tenant_id: userProfile.tenant_id
      }]).select().single();
      ...
      const { data: newCb } = await supabase.from('cashbook').insert([{
        ...
        amount, tenant_id: userProfile.tenant_id, reference_id: newInv.id
      }]).select().single();
      setData(prev => ({
        ...prev,
  ...
  return {
    ...
    handleTransfer,
    handleAddEditInvestor, handleDeleteInvestor, handleAddInvestment,
